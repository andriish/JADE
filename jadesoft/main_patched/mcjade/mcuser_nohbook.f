...
CAV      CALL HCDIR('//PAWC',' ')
CAV      CALL HSTAF('YES')
CAV      CALL HBOOK1(IDNE,'N OF NEUTR PARTICLES (INPUT)',100,0.,100.,0.)
CAV      CALL HBOOK1(IDCH,'NO OF CHRGD PARTICLES (INPUT)',100,0.,100.,0.)
CAV      CALL HBOOK1(IDTRK,'NO OF CHRGD PARTICLES (PATR)',100,0.,100.,0.)
CAV      CALL HBOOK1(IDHIT,'NO OF I.D. HITS',100,0.,1500.,0. )
CAV      CALL HBOOK1(IDLG,'LG TOT ENERGY',50,0.,50.,0. )
CAV      CALL HBOOK1(IDCAP,'LG END CAP ENERGY',50,0.,50.,0. )
CAV      CALL HBOOK1(IDBAR,'LG BARREL ENERGY',50,0.,50.,0. )
CAV      CALL HBOOK1(IDPTOT,'SUM PTOT',50,0.,50.,0. )
CAV      CALL HBOOK1(IDPT,'SUM PT',50,0.,50.,0. )
CAV      CALL HBOOK1(IDTOT,'SUM PTOT + LG',50,0.,50.,0. )
C
      RETURN
      END
C
...
CAV      CALL HF1( IDNE, REAL( IW( IND +  6 ) ), 1. )
CAV      CALL HF1( IDCH, REAL( IW( IND +  5 ) ), 1. )

C
C...  # OF PATR TRACKS
 1000 IND = IW( IBLN('PATR') )
      IF( IND .EQ. 0 ) GO TO 2000
CAV      CALL HF1(IDTRK,REAL( IW( IND +  2 ) ),1.)
C
...
CAV      CALL HF1( IDPTOT, PTOT ,1. )
CAV      CALL HF1( IDPT, PT ,1. )
C
C...  # OF JETC HITS
 2000 IND = IW( IBLN('JETC') )
      IF( IND .EQ. 0 ) GO TO 3000
      NHITS = ( HW(IND*2+99) - HW(IND*2+3) ) / 4
CAV      CALL HF1( IDHIT, REAL(NHITS) ,1. )
C
C...  LG ENERGY
...
C
 ...
 3010 ETOT = ETOT + HW(I+2)
      ...
      ETOT = ETOT * .001
C
CAV      CALL HF1( IDLG, ETOT, 1. )
CAV      CALL HF1( IDCAP, ECAP, 1. )
CAV      CALL HF1( IDBAR, ETOT - ECAP, 1. )
C
C... TOTAL ENERGY
CAV      CALL HF1( IDTOT, ETOT + PTOT, 1. )
C
 4000 RETURN
      END
C
