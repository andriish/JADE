C   06/01/82 206161251  MEMBER NAME  TMUC82   (S)           FORTRAN
...
C
      CALL NEWMOM(I1,I2,0)
C     IF(ABS(DZ1(I1)).LT..75.AND.ABS(DZ1(I2)).LT..75) GOTO  65
C     EVWRIT = .FALSE.
C     CALL HF1(3508,90.,1.)
C     NQUMU = 0
C     IF(IQUMU(I1).GT.0)  NQUMU = NQUMU + 1
C     IF(IQUMU(I2).GT.0)  NQUMU = NQUMU + 1
C     IF(LHIST.NE.0) CALL HFILL(ID+8,DZ1(I1),FLOAT(NQUMU))
C     IF(NQUMU.GT.0)  EVWRIT = .TRUE.
C     IF(EVWRIT) CALL HF1(3508,92.,1.)
C     IREJ = 27
