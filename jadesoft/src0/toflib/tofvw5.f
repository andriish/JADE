C   14/05/80 409060914  MEMBER NAME  TOFVW5   (S)           FORTRAN
C
C-----------------------------------------------------------------------
      SUBROUTINE TOFVW
C-----------------------------------------------------------------------
...
C
  38  NTRR = INFM(1)
        IF( NTRR .NE. 0 ) GO TO 40
        CALL TRMOUT(80,'NO TRACKS TO DISPLAY^')
        GO TO 998
C
  40  DO  100  ITR = 1,NTRR
        IF(IABS(IR(2,ITR)).NE. 1) GO TO 100
C TRACK NUMBER
        LTR = IR(1,ITR)
        BETA  = RE(13,ITR)
        TOF   = RE(4,ITR)
...
C
        ...
        CALL DNUM( LTR, PBETA + 0.02, BETA + 0.02, DESIZE, 0.0 )
C
 100  CONTINUE
C
