C   22/05/82 404191117  MEMBER NAME  TOFMAS6  (S)           FORTRAN
...
C  CREATES BANK TOFR
C  THIS VERSION USES NEW 1982 CALIBRATION WITH INDIVIDUAL SCINTILL
C  VELOCITIES VSC
C HAS TO BE USED TOGETHER WITH SUBCAL6
...
C
      DATA IENTRY/0/,IBUG/10/,NEXIST/0/
...
C
      ...
      IENTRY = IENTRY + 1
C     CALL CNTFIT(1,1,Z1,TAUM1,TAUP1)
   66 ZTOF = 0.
C
      NRUN = HEADR(18)
C
      TPARM(25) = 0.
      IF(NRUN.GE.10265.AND.NRUN.LE.10365) TPARM(25) =-10.66
      IF(NRUN.GE.10265.AND.NRUN.LE.10365) TPARM(25) =-10.66
      CC2M = 0.
      CC2P = 0.
      IF(NRUN.LE.10811)CC2M = -4.25
      IF(NRUN.LE.10811)CC2P = -2.35
      IF(NRUN.GT.10811)CC2M = -1.90
      IF(NRUN.GT.10811)CC2P = -1.90
C     CC2M = -4.25
C     CC2P = -2.35
C
      ZTOF1 = 0.
...
C
      ...
      PATH1= TRK(3,NTR1)
      PMEV1= TRK(4,NTR1)
      TAUM1= RAW(2,I)-Z1*VSC(2*I-1)
      TAUP1= RAW(3,I)-Z1*VSC(2*I  )
C     TAUM1 = TAUM1 - (TPARM(7)*PATH1+TPARM(8))
C     TAUP1 = TAUP1 - (TPARM(7)*PATH1+TPARM(8))
      IF(IBUG.LE.10)PRINT 230,I,RAW(2,I),Z1,PATH1,TAUM1,VSC(2*I-1)
      IF(IBUG.LE.10)PRINT 230,I,RAW(3,I),Z1,PATH1,TAUP1,VSC(2*I)
  230 FORMAT(' TOFMAS6',I5,10F10.4)
C     PCOR = (PATH1-RADTOF)/CVEL
C     TAUM1 = TAUM1 - PCOR
C     TAUP1 = TAUP1 - PCOR
      IF(NRUN.LT.4993) GOTO  321
C  BEAM CURRENT CORR
C     TAUM1 = TAUM1 - CPOS *TPARM(21)
C     TAUP1 = TAUP1 - CPOS *TPARM(22)
C  AD HOC CORR FOR BEGIN 1981
      TAUM1 = TAUM1 - TPARM(25) +3.07
      TAUP1 = TAUP1 - TPARM(25) +3.07
      IF(I.EQ.2) TAUM1 = TAUM1 - CC2M
      IF(I.EQ.2) TAUP1 = TAUP1 - CC2P
  321 CONTINUE
C
      NFLG= -1
...
C
  330 CONTINUE
      IF(ABS(TAUM1-TAUP1).LE.3.) GO TO 350
...
C
      VWM = 1.
      VWP = 1.
      IF(ADCM.GT.30.) VWM = TPARM(9)/ADCM+TPARM(10)
      IF(ADCM.GT.30.) VWP = TPARM(9)/ADCP+TPARM(10)
      VWM = 1./(VWM*VWM)
      VWP = 1./(VWP*VWP)
