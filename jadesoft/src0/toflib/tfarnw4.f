C   11/01/81 906051754  MEMBER NAME  TFARNW4  (SD)          FORTRAN
...
#include "tfadc.for"
     ...
     - ,ITRC(50),NTC,ITRK(5,42),INFM(4),IR(14,50)
      ...
      DIMENSION IRAW(5,42),HEADR(108),LIST1(2),LIST(19),LIST2(8)
...
C
     ...
     1          41/
      DATA LIST2/17,21,22,24,25,28,31,32/
C
      DATA IENTRY/0/,IBUG/20/
...
C     IF(NRUN.GE.7897.AND.L.GT.7)  GOTO  77
   ...
   77 CONTINUE
C
      IF(NRUN.LT.8712)  GOTO  99
      IF(NRUN.GT.10000)  GOTO  87
      IF(NCN.NE.8)  GOTO  88
      HTDC(1,NCN) = 2050
      MONE = 2
   88 CONTINUE
...
C
   ...
   91 CONTINUE
C
      IF(NRUN.LT.14605)  GOTO  99
      IF(NRUN.GT.14687)  GOTO  94
      IF(NCN.NE.36)  GOTO  94
      HTDC(1,NCN) = 2050
      MONE = 2
   94 CONTINUE
C
      IF(NRUN.LT.15837)  GOTO  99
      IF(NRUN.GE.15955)  GOTO  95
      IF(NCN.NE. 4)  GOTO  95
      HTDC(2,NCN) = 2050
      MONE = 1
   95 CONTINUE
C
      IF(NRUN.LT.16803)  GOTO  99
      IF(NRUN.GE.17212)  GOTO  96
      IF(NCN.NE.19)  GOTO  96
      HTDCL = HTDC(2,NCN)
      HTDC(2,NCN) = HTDC(1,NCN)
      HTDC(1,NCN) = HTDCL
C
   96 IF(NRUN.LT.17030)  GOTO  99
      IF(NRUN.GE.17067)  GOTO  97
      IF(NCN.NE.4.AND.NCN.NE.5)  GOTO  97
      HTDC(2,NCN) = 2050
      MONE = 1
C
   97 CONTINUE
      IF(NRUN.LT.18000)  GOTO  99
      IF(NRUN.GT.19025)  GOTO  121
      IF(NRUN.LT.18894)  GOTO  82
      DO   80   LL=1,LBAD
      IF(NCN.EQ.LIST2(LL)) GOTO  81
   80 CONTINUE
      GOTO  82
   81 HTDC(2,NCN) = 2050
      MONE = 1
      GOTO  99
   82 IF(NCN.NE.14)  GOTO  99
      HTDC(1,NCN) = 2050
      MONE = 2
  121 CONTINUE
      IF(NRUN.LT.20000)  GOTO  99
      IF(NRUN.GT.21705)  GOTO  122
      IF(NCN.NE.14)  GOTO  131
      HTDC(1,NCN) = 2050
      MONE = 2
  131 IF(NCN.NE.27)  GOTO  132
      HTDC(2,NCN) = 2050
      MONE = 1
  132 IF(NCN.NE.28)  GOTO  122
      HTDC(2,NCN) = 2050
      MONE = 1
  122 CONTINUE
      IF(NRUN.LT.24214)  GOTO  99
      IF(NRUN.GT.30500)  GOTO  123
      IF(NCN.NE.19)  GOTO  123
      HTDC(2,NCN) = 2050
      MONE = 1
  123 CONTINUE
      IF(NRUN.LT.28250)  GOTO  99
      IF(NRUN.GT.30500)  GOTO  99
      IF(NCN.NE. 6)  GOTO  99
      HTDC(2,NCN) = 2050
      MONE = 1
C
...
C CHANGED JUNE 88
C     TDCM = HTDC(1,NCN)-HTSPAR(3)+465.
C     TDCP = HTDC(2,NCN)-HTSPAR(3)+465.
      TDCM = HTDC(1,NCN)
      TDCP = HTDC(2,NCN)
      IF(NRUN.GE.24200) TDCM = HTDC(1,NCN)-HTSPAR(3)+465.
      IF(NRUN.GE.24200) TDCP = HTDC(2,NCN)-HTSPAR(3)+465.
C
...
C     CALL HFILL(702,CORADP,FLOAT(NCN))
