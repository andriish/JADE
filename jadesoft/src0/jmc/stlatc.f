C   18/04/79 607171120  MEMBER NAME  STLATC   (S)           FORTRAN
C
C-----------------------------------------------------------------------
      SUBROUTINE STLATC( LENGTH, ILTC )
C-----------------------------------------------------------------------
C
C   AUTHOR:   E. ELSEN     18/04/79 :  PROVIDE DATA FOR BANK 'LATC' AS
C                                      DESCRIBED IN JADE NOTE 32.
C
C        MOD  E. ELSEN     29/05/79 :
C   LAST MOD  J. HAGEMANN  21/09/84 :  IF VERTEX CHAMBER TRACKING SET ->
C                                      NO BPC-DATA
C
...
CCC  LATCHES FOR BP COUNTERS
      IF( LFLAG(5) ) GO TO 150
         DO 100 J =1,24
         IF( HBPCAR(J) .EQ. 0 ) GO TO 100
            NW = (J-1) / 8
            IBT = J - NW*8
            HLATCH(NW+3) = LOR( HLATCH(NW+3), MASK(IBT) )
  100    CONTINUE
C
CCC  LATCHES FOR TOF COUNTERS
  150 DO 200 J =1,42
      ...
      NW = (J-1) / 7
      IBT = J - NW*7
      HLATCH(NW+6) = LOR( HLATCH(NW+6), MASK(IBT) )
  200 CONTINUE
C
...
CCC  STORE LG ROW LATCHES
      ...
      NW = (J-1) / 7
      IBT = J - NW*7
      HLATCH(NW+12) = LOR( HLATCH(NW+12), MASK(IBT) )
 1100 CONTINUE
C
CCC  STORE END CAP LATCHES
      DO 1200 J =1,8
 1200 IF( NLGQ(J) .GT. LGQT ) HLATCH(18) = LOR( HLATCH(18), MASK(J) )
C
CCC  STORE TOTAL LG ENERGY SUM
      DO 1300 J =1,4
 1300 IF( NLGE .GT. LGET(J) ) HLATCH(22) = LOR( HLATCH(22), MASK(J) )
C
C
