C   07/11/86 707080940  MEMBER NAME  ZE4VTP   (S)           FORTRAN77
      SUBROUTINE ZE4VTP( IGEN )
C-----------------------------------------------------------
C VERSION OF 11/11/86       M.ZIMMER
C MODIFIED   27/11/86       M.ZIMMER
C MODIFIED   18/02/87       G.ECKERLIN
C MODIFIED   02/04/87       G.ECKERLIN (BUG CORRECTED)
C LAST MOD   08/07/87       G.ECKERLIN  NO CALL TO ZE4EK FOR MC
...
C
      DIMENSION R(3), RD(3), VECTB(1000), HRUN(2), PMOM(3), RDOCA(3)
...
C                                  ERROR IN BCRE
 30   CONTINUE
      WRITE (6,9030) IER
 9030 FORMAT(1X,'ERROR CREATING ZEHD BANK : ',I4)
      LVECS = 0
      GOTO 1
...
C                                      LOOK FOR BANKS
      ...
      NTR = HW( NPTPEV*2 + 5 )
      IF( NTR .LE. 0 )   CALL FEHLER( 2, &8000 )
      NVX = HW( NPTPEV*2 + 12 )
      IF ( NVX .LT. 1 )     CALL FEHLER( 3, &8000 )
      NPPATR = IW( IBLN('PATR'))
      IF( NPPATR .LE. 0 )    CALL FEHLER( 4, &8000 )
C
C                                      IN ADDITION ONE RUN-VERTEX
      NVX = NVX + 1
C                                      IF NOT MC LMC = 0
...
C                                           SET VERTEX HERE
      KPTPVX = IW( IBLN( 'TPVX'))
      IF( KPTPVX .LE. 0 ) CALL FEHLER( 5, &3546 )
 3546 CONTINUE
      CALL BPOS( 'TPVX' )
C                                      FILL VERTEX
C                                      FILL RUN-VERTEX AS FIRST VERTEX
      NTRVX = 0
      CALL ZE4VVX( RD, NTRVX )
      DO 1010 I = 1,3
        RW(NPZE4V + LH + I) = RD(I)
 1010 CONTINUE
C                                      FILL EVENT-VERTEX AND SEC. VERT.
CCC   DO 1020 I0 = 1,NVX
      DO 1020 I0 = 1,NVX-1
        ...
        DO 1020 I=1,3
          ...
          ELSE
            R(I) = RW( NPTPVX + 1 + I )
          ENDIF
          RW(NPZE4V + LH + LVX*(I0) + I) = R(I)
CCC       RW(NPZE4V + LH + LVX*(I0-1) + I) = R(I)
 1020 CONTINUE
C                                           MC PART OF HEADER
...
C
        IF ( ABS( CHARGE ) .GT. .5 .AND. NPPATR .GT. 0 )     THEN
          ...
          IF (NRPATR.LE.0) THEN
            WRITE (6,*) 'TRACK NOT IN PATR BANK, TRACK :',J
            RMIN = RW(NPTPTR + 16)
            CALL FEHLER(33,&300)
          ENDIF
          ...
          NRPHI = IW (NPTRAK + 24)
          NRZ   = IW (NPTRAK + 33)
          IF( NRPHI .LT. NRPHCT )   CALL FEHLER( 12, &300 )
          IF( NRZ .LT. NRZCT )         CALL FEHLER( 13, &300 )
C         RMIN = RW( NPTPTR + 16 )
          IF( ABS( RMIN ) .GT. RMINCT ) CALL FEHLER ( 14, &300 )
          AZV = ABS( RW( NPTRAK + 31) )
          IF( AZV .GT. ZCT )           CALL FEHLER( 15, &300 )
C                        GET MOMENTUM FROM CRDOCA FOR CHARGED PART.
C                        WITH REPECT TO THE RUN VERTEX FROM BUPDAT
          CALL CRDOCA( NPTRAK, RD, PMOM , RDOCA, CHARGE)
        ENDIF
C                                      ACCEPT THIS TRACK
...
CCC     IF( HW(NPZE4V*2+5).LT.100 .AND. NPPATR .GT. 0)
        IF( MCFLAG.NE.0 .AND. NPPATR.GT.0 .AND. ABS(CHARGE).GT..5)
...
C                                      LG-INFORMATION
        ...
        IF ( NPLGCL .GT. 0 ) THEN
          ...
          IF ( NPOINT .GT. ( NPLGCL + IPENDE )) CALL FEHLER( 16,&3123)
          ECL    = RW( NPOINT + 2 )
          SIGECL = RW( NPOINT + 3 )
          JBAREC = IW( NPOINT + 1 )
          ICON = IW(NPOINT + 8)
...
C                                      ELECTRON SEARCH
          IF (MCFLAG.EQ.0) THEN
            CALL ZE4VEK( NP , IEKA )
            IF ( IEKA .EQ. 0 ) ITYP = 2
CCC         IF ( IEKA .NE. 0 ) WRITE(6,*) 'ZE4VEK CODE : ',IEKA
          ENDIF
C                                      PARTICLE CODE (MC-CONV)
          HW( NP2 + 8) = ITYP
C
...
C                                       CUT IN PTOT
          IF( RW(NP + 6) .LT. ELGPHM )  THEN
            CALL BCHM( NPZE4V, (-1)*LT, IER)
            CALL FEHLER ( 19, &300 )
            WRITE(6,*) '!!!!  PTOT NOT PASSED !!!!!!!!!!!!!!!!!'
          ENDIF
C                                      SET DET. FLAG = 100 FOR BARREL
...
C
C                                   SCIP IF IMODE LT 2
      IF( IMODE .LT. 2 ) GO TO 2050
C
      IF( NPLGCL .LE. 0 ) GO TO 2050
C
      NCLST = IW( NPLGCL + 7 )
...
C                                           PATH LENGTH CORREC
        ...
        ETOT = ETOT - ELGMIN*(ICONN/100)*DZDS
