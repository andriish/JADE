C   14/12/81 808021131  MEMBER NAME  LGECOR   (SOURCE)      FORTRAN
C
C    11.7.88 D.PITZL : CHANGED TO AGREE WITH NEW VERSION OF THCORR:
C        RTH = ELOST / EINC   NOW
C        RTH = ELOST / ESEEN  PREVIOUS VERSION

C    CHANGED 21.12.87 D.PITZL:
C     ENLOSG CALLED FOR PHOTONS   ( IDATA(8) = 0 ) : ENERGY LOSS IN AL
C     ENGLOS CALLED FOR ELECTRONS ( IDATA(8) > 0 ) : ENERGY LOSS IN AL
C     THCORR CALLED : CORRECTION FOR LG-READOUT THRESHOLD
C     LKCORR CALLED : CORRECTION FOR LEAKAGE OF PHOTON-SHOWERS
C        NOT CALLED FOR MC-DATA WITH MEIER-MAGNUSSEN TRACKING
C        ( WORD 17 IN HEAD-BANK = 1 )
C        SINCE THE LEAKAGE OF PHOTON SHOWERS IS NOT INCLUDED THERE.
C
C
C****************************************************************
      SUBROUTINE LGECOR( RDATA, DEP, IFLAG)
C****************************************************************
C
...
C    RDATA(9--11) DIRECTION COSINES CORRECTED FOR SHOWER DEPTH
C
...
C
 ...
 3075 FORMAT(/T5,'JADELG.LOAD (LGECOR) VERSION 22.12.87 CALLED',
     +    ' INCLUDING LKCORR, THCORR, ENLOSG'/)

      IF ( ICALLS .EQ. 1  .AND.  NRUN .LT. 100 .AND. MCTYP .EQ. 1 )
     +   PRINT 3076
 3076 FORMAT(/T2,'JADELG.LOAD (LGECOR) VERSION 22.12.87 CALLED',
     +   ' FOR MEIER-MAGNUSSEN MC-DATA, INCLUDING THCORR, ENLOSG'/)

      IF ( ICALLS .EQ. 1  .AND.  NRUN .LT. 100 .AND. MCTYP .NE. 1 )
     +   PRINT 3077
 3077 FORMAT(/T2,'JADELG.LOAD (LGECOR) VERSION 22.12.87 CALLED',
     +   ' FOR MC-DATA, INCLUDING LKCORR, THCORR, ENLOSG'/)
C
C  INITIAL SET UP
C
      BE     = RDATA(1)
...
C
C                                IF PHOTON CLUSTER, CALL ENLOSG
      IF ( NIDTRK .EQ. 0 ) CALL ENLOSG (EINC,D,DELTAE,&80)
C                                IF CHARGED CLUSTER, CALL ENGLOS
      IF ( NIDTRK .GT. 0 ) CALL ENGLOS (EINC,D,DELTAE,&80)
C
      EEXP=EINC-DELTAE
...
C             NOT CALLED FOR MC-DATA WITH MEIER MAGNUSSEN LG-TRACKING
C              (MCTYP = 1 ) SINCE LEAKAGE IS NOT SIMULATED THERE
C
      EINCLG = EEXP * 1000.
      RLEAK = 0.0
      IF ( NRUN.GT.100.OR.(NRUN .LT. 100  .AND. MCTYP .NE. 1) )
     +   CALL LKCORR ( EINCLG, NRING, RLEAK )
      EEXP = EEXP * ( 1. - RLEAK )
C
C              CORRECTION FOR READOUT THRESHOLD
C              RTH = LOST ENERGY / INCIDENT ENERGY
C
      RTH = 0.0
      CALL THCORR ( EINCLG, NRING, RTH )
CCC   EEXP = EEXP / ( 1. + RTH )    <<== OLD VERSION OF THCORR
      EEXP = EEXP * ( 1. - RTH )
C
C              ANGLE DEPENDENCE
C
...
C
   50 CONTINUE
      PRINT 7122,ITERA,ENERGY,EINC,NRUN,NEVE
 7122 FORMAT (T2,'ITERATION OVERFLOW IN LGECOR: #ITERA, ESEEN, ECALC (ME
     +V) =', I4,3P,2F10.1,'   RUN ',0P,I5, '  EVENT ',I5)
      IFLAG=1
      RETURN
C
