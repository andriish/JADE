C   12/03/82 406181402  MEMBER NAME  BRLGNT   (SOURCE)      FORTRAN
      FUNCTION BRLGN( EINC, COSTH, PHI )
C        FINE POSITION DEPENDENCE OF LG-GAIN IN BARREL-PART
C                            19-MAR-82  S.ODAKA
C        LAST MOD.           16-JUN-84  S.YAMADA     INCLUDE NEW SF6
C
...
C
      ...
      DATA INIT/0/
C
C  **************** POSITION DEPENDENCE DEPENDS ON THE PERIOD *******
C
      NPHEAD = IDATA( IBLN('HEAD') )
      IF( NPHEAD.GT.0 ) GO TO 30000
C
      WRITE(6,6001)
 6001 FORMAT('0***** ERROR IN BRLGN/LGECOR/LGCDIR;  NO /HEAD/ BANK',
     1'     TREATED AS NEW DATA AFTER 1983')
      GO TO 20000
C
30000 NPHED2 = 2*NPHEAD
      NEXP = HDATA( NPHED2+9 )
      IF( NEXP.LT.24 ) GO TO 10000
C
C     1983---
20000 BRLGN = BRLGN6( EINC, COSTH, PHI )
      RETURN
C
C
C     1979---1982
