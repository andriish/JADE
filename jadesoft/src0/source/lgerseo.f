C   16/07/79 002181457  MEMBER NAME  LGERSE   (SOURCE)      FORTRAN
...
C  CODED BY H.TAKEDA  09/05/79  14:30
C       LAST MODIFICATION 23/05/79  13:15
C       LAST MODIFICATION BY Y.WATANABE 23-09-79 23:00
C
...
30    IF(HIT.LT.HABAD(1,2)) GO TO 62
      IF(HCHB(1).GT.0) GO TO 32
      IF(HCHB(2).GT.0) GO TO 32
      IF(HCHB(3).GT.0) GO TO 32
      GO TO 62
32    HCR(1)=0
      HCR(2)=0
      HCR(3)=0
...
C     DO 150 I=1,HIT
C     IZ = HLGADC(1,I)
C     IF(IZ.GT.2687) GO TO 150
C     IZ=MOD(IZ,32)
C     IF(IZ.EQ.0.OR.IZ.EQ.31) HLGADC(2,I)=0
C150   CONTINUE
      RETURN
C
C  RUN # NOT FOUND
