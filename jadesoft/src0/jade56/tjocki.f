C   16/05/87 705281927  MEMBER NAME  TJOCKI   (S)           FORTRAN
      SUBROUTINE TJOCKI(COSTHE,XLEN90,XLEN,ELODIF)
C
...
C
C    OUTPUT: XLEN, THE ACTUAL RADIATION LENGTH
C            XLEN90, CORRESPONDING RAD.LENGTH AT RIGHT ANGLE
C            ELODIF, ENERGY LOSS DIFFERENCE, FOR USE IN LSCCTC
C            IT GIVES THE DIFFERENCE IN SPECIFIC ENERGY LOSS FOR
C            ELECTRONS FOR THE ACTUAL COIL MATERIAL IN THIS DIRECTION,
C            AS COMPARED TO A PURE AL-ABSORBER, WHICH IS USED IN THE
C            SF56 TOKYO SHOWER PROGRAM.
C
      INTEGER*2 HDATE
...
C
C ELOSS DIFFERENCES FOR: 1. AL - WATER, 2. AL-LUCITE(RESIN),
C                        3. AL - POLYSTYRENE (SCINTILLATOR)
C  DATA FROM BERGER-SELTZER (1964), NASA SP-3012
C
      DATA XDIF1 /32.6338/
      DATA XDIF2 /38.2631/
      DATA XDIF3 /44.6646/
...
C
      ...
      DATA XALUMI /89./
C
C RAD.LENGTH FOR REST OF ABSORB MATERIAL FROM JCN 87
...
C    19784-1986 VALUE
      DATA XRESI2 /.3860/
C
C FOR CALCULATION OF ELODIF, THE PERCENTAGE OF AL,H2O,RESIN,SCINTIL.
C IN THE NON-COIL MATERIAL IS ALSO NEEDED:
C
C
      DIMENSION DIFPRO(4,2)
      DATA DIFPRO/0.683,0.0,0.170,0.147,
     $            0.573,0.0,0.304,0.123/
C
...
C CALCULATE ELODIF
C
      WATPRT = (XLH2O/XWATER + DIFPRO(2,IFL86)*XRESJD)/XLEN
      RESPRT = (XRES/XRESIN  + DIFPRO(3,IFL86)*XRESJD)/XLEN
      SCIPRT = (        DIFPRO(4,IFL86)*XRESJD)/XLEN
C
      ELODIF = WATPRT*XDIF1 + RESPRT*XDIF2 + SCIPRT*XDIF3
      ELODIF = ELODIF/RME
C
C
9999  CONTINUE
