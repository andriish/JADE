C   27/04/81 705161141  MEMBER NAME  LGCLXAR  (S)           FORTRAN
...
C--       FOR REAL DATA ONLY, "NEGATIVE LEAKAGE" CORRECTION
C--       RECREATE LGCL, UPDATE RESO
C                                                 LAST CHANGE 16.05.87
...
C
      CALL BDLS('LGCL',1)
C
C  SCALE ALL ENERGIES IN ALGN WITH FACTOR SCLGCL AND LEAKAGE CORR.
C
      IALGN=IBLN('ALGN')
      IPJ=IDATA(IALGN)
      IF(IPJ.LE.0) GO TO 1
...
C
      ...
      GO TO 5010
C
5009  EBLO = SCLGCL * EBLO
C
5010  HDATA(IJK) = IFIX(EBLO)
C
5004  CONTINUE
C
      INDEX=INDEX+1
