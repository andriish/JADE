C   05/05/81 105201638  MEMBER NAME  USREPAT  (JADESR)      FORTRAN
...
C     CALL BDLS(NA,NR)
   ...
   15 CONTINUE
      CALL BGAR(IGA)
      IF(IGA.NE.1) WRITE(6,102) NREC,NRUN,NEVT
  102 FORMAT(1X,' GARBAGE COLLECT. FAILED. SEQ NO RUN AND EVT = ',3I6)
      CALL JETCAL
      IND=0
      CALL PATREC(IND)
      IF(NREC.GT.-1) GO TO 11
      INDEX=INDEX+1
      RETURN
C
 1003 CONTINUE
