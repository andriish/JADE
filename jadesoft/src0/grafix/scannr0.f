C   10/06/86 606101844  MEMBER NAME  SCANNR0  (S)           FORTRAN
C
C-----------------------------------------------------------------------
...
C   LAST MOD: C. BOWDERY  10/06/86 :  NEW COMMON FOR STANDARD VIEW DATA
C
...
C
 2000 IF( LSTCMD .EQ. 112  .AND.  ICREC .NE. ICRSTR ) GO TO 2001
      ...
      IF( HEAD(18) .EQ. NRREQU  .AND.  HEAD(19) .EQ. NEREQU ) GO TO 200300008800
...
C
     ...
     +    ISTANV .EQ. 10  .OR.  ISTANV .EQ. 11     ) DSPDTL(9) = .FALSE.
      CALL ERASE
      CALL EVDISP(ISTANV)
      IF( HCSTV3 .EQ. 1 ) CALL JADISP(ISTANV)
...
C
   5  CALL TRMOUT(80,'Software Error!  Please contact graphics expert^')
      ...
      RETURN
C
 1110 GO TO ( 101, 102, 103, 104, 105, 106, 107, 108, 109, 110) , ICOMD200015400
 1120 GO TO ( 111, 112, 113, 114, 115, 116, 117, 118, 119, 120) , ICOMD200015500
 2130 GO TO ( 121, 122, 123, 124, 125, 126, 127, 128, 129, 130) , ICOMD200015600
 3140 GO TO ( 131, 132, 133, 134,   5,   5,   5,   5,   5,   5) , ICOMD200015700
C
C
...
C
 107  CALL TRMOUT(80,'Please enter NAME of bank to be DELETED:^')
      CALL TRMIN(4,NAME)
      IPNAME = IDATA(IBLN(NAME))
...
C
 1072 IF(IDATA(LIM1+1).EQ.0) GO TO 1077
      CALL TRMOUT(80,'The following BOS bank numbers are present:^')
      LM1 = LIM1
