...
C
  20  IF( LSTCMD .EQ. 52  .OR.  FL18 ) RETURN
C
...
C
 295  XXX  = XMAX - 0.57*(XMAX - XMIN)
      ...
      BFLD  = HEAD(38)*0.001
      CALL CORE(HMW,44)
      WRITE(JUSCRN,300) EBEAM,BFLD,HEAD(30)
...
C
 ...
 370  IBWSC  = LAND(IDATA(ITRG+1),MASK2)
     ...
     +                          WRITE(JUSCRN,372) HRGW
      IF( IBWSC  .NE. 0  .AND.  IBWSC4 .EQ. 0 )
     +                          WRITE(JUSCRN,374) HRGWA,HRGWP
      IF( IBWSC4 .NE. 0 )       WRITE(JUSCRN,376) HRGWA1,HRGWP1,HRGWT2
...
C
      ...
      CALL MOVEA(XXX,YYY)
      CALL CORE(HMW,32)
      WRITE(JUSCRN,500) (HEAD(I),I=11,16)
