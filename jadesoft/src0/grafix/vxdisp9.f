C   01/11/84 803241601  MEMBER NAME  VXDISP9  (S)           FORTRAN
C
C-----------------------------------------------------------------------
...
C  LAST MOD:   J. O., J.S. 21/04/87 :  HELIX DISPLAY IN Z-VIEWS
C
...
112   IF(NN.NE.1.OR.FL18) GO TO 111
      ...
      NDF = 2*IV(INVP+8) - 3
      CALL CORE(HWORK,80)
      WRITE(10,219) INV,IV(INVP+1),IV(INVP+10),V(INVP+9),NDF
219   FORMAT(' ',I2,' FLAG',I2,' NTR ',I2,' CHI/NDF ',F6.2,'/',I2)
      CALL SYSSYM(XXX,YYY,SSS,HWORK,35,0.)
      YYY = YYY - 1.5*SSS
      CALL CORE(HWORK,80)
      WRITE(10,218) V(INVP+2),V(INVP+3),V(INVP+4)
...
C **MODIFY TO DRAW HELIX FIT RESULT    J.S. 16/4/87
     ...
     +CALL HELDIS(ITR,INDEX,VXE,VYE,VZE,
     +ADATA(IQO+12),ADATA(IQO+13),ADATA(IQO+14),ADATA(IQO+25))
C    +CALL HELDIS(ITR,INDEX,ADATA(IQO+5),ADATA(IQO+6),ADATA(IQO+7),
C    +ADATA(IQO+12),ADATA(IQO+13),ADATA(IQO+14),ADATA(IQO+25))
      IF(ITYPLI.EQ.1.OR.ITYPLI.EQ.2.AND.LLLL.EQ.0)
     +CALL LINDIS(IQO,INDEX,VXE,VYE,VZE)
C ** END MODIFICATION
C     IF(ITYPLI.EQ.1) CALL LINDIS(IQO,INDEX,VXE,VYE,VZE)
...
1161  VXG = 0.
      VYG = 0.
      VZG = 0.
      IF(NTR.LE.0.OR.IV(1).EQ.4) GO TO 1920
