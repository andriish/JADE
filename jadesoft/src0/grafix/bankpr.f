...
C****************************** WRITE BANK HEADER
      ...
      NAME  = IDATA(LIM1-1)
      CALL CORE(HDSPL,76)
      WRITE(10,140) (IDATA(LHO),LHO=LIM1,LIMX),(HDATA(LIMHX+J),J=1,2)
...
C--------------- SPECIAL FORMAT FOR PATR BANKS
 2000 LIM2  = LIM2 - 1
      ...
      IPO   = LIM2 + LO
      CALL CORE(HWORK,47)
      WRITE(JUSCRN,2001) (IDATA(LIM2+II),II=1,LO)
 ...
 2004 IPP = IPO + (ICNT-1)*LTR
      CALL CORE(HWORK,122)
      WRITE(JUSCRN,2005) IDATA(IPP+1),IDATA(IPP+2),
 ...
 2005 FORMAT(' ',I2,1X,Z8,1X,I5,I3,1X,2F8.3,F10.3,3F7.3,2X,I3,1X,
     ...
     &WRITE(JUSCRN,2006) IDATA(IPP+18),(ADATA(IPP+I),I=19,23),
 ...
 2006 FORMAT(' ',I2,1X,E10.4,F8.3,E11.4,1X,F8.3,F7.3,I3,1X,4E11.4,1X,I2,
     ...
     &WRITE(JUSCRN,2007) IDATA(IPP+18),(ADATA(IPP+I),I=19,23),
     &                   IDATA(IPP+24),(ADATA(IPP+I),I=25,28),
     &                   IDATA(IPP+29),(ADATA(IPP+I),I=30,32),
     &                   IDATA(IPP+33)
 2007 FORMAT(' ',I2,1X,E10.4,2F8.3,1X,E11.4,F7.3,I3,1X,4E11.4,1X,I2,
     & 3F7.2,I3)
      ...
      IF( NPATR .NE. 12 ) GO TO 2009
         CALL CORE(HWORK,118)
         WRITE(JUSCRN,2008) (IDATA(IPP+I),I=34,48),
     &                      (HDATA(IPP*2+I),I=3,4)
 ...
 2009 CALL CORE(HWORK,89)
      WRITE(JUSCRN,2010) (IDATA(IPP+I),I=34,44),(ADATA(IPP+I),I=45,46),
     &                   (IDATA(IPP+I),I=47,48)
 ...
 2020 CONTINUE
      YS = YS - DEL
      CALL CORE(HWORK,124)
      WRITE(JUSCRN,2021) (ADATA(IPP+I),I=49,59),
     &                   (IDATA(IPP+I),I=60,62)
...
C--------------- SPECIAL FORMAT FOR TAGC BANK
 ...
 3003    CONTINUE
         IILIM = 6*ILIM + 1
         CALL CORE(HWORK,IILIM)
         WRITE(JUSCRN,3004) (HWORK(LLO+100),LLO=1,ILIM)
 3004    FORMAT(' ',20I6)
         CALL SYSSYM(XS,YS,SIZE,HWORK,IILIM,0.)
 3005 CONTINUE
      GO TO 90100
C--------------- SPECIAL FORMAT FOR JHTL BANK
 ...
 4002    LIML = LIM2 + 4*(I-1) + 1
         ...
         DO 4003  J = 1,ILIM
            ...
            IWD  = IDATA(LLO)
            IJK  = (J-1)*10 + 100
            HWORK(IJK+ 1) = LAND(SHFTR(IWD,27), 31)
            HWORK(IJK+ 2) = LAND(SHFTR(IWD,25),  3)
            HWORK(IJK+ 3) = LAND(SHFTR(IWD,24),  1)
            HWORK(IJK+ 4) = LAND(SHFTR(IWD,17),127)
            HWORK(IJK+ 5) = LAND(SHFTR(IWD,16),  1)
            HWORK(IJK+ 6) = LAND(SHFTR(IWD,11), 31)
            HWORK(IJK+ 7) = LAND(SHFTR(IWD, 9),  3)
            HWORK(IJK+ 8) = LAND(SHFTR(IWD, 8),  1)
            HWORK(IJK+ 9) = LAND(SHFTR(IWD, 1),127)
...
C--------------- SPECIAL FORMAT FOR GVTX BANKS
 6000 NV  = IDATA(LIM2)
      ...
      DO 6002  INV = 1,NV
         IPO = LIM2+(INV-1)*10
         CALL CORE(HWORK,72)
         WRITE(JUSCRN,6001) IDATA(IPO+1),(ADATA(IPO+1+JJ),JJ=1,6),
 ...
 6003 FORMAT(' ',I3)
      ...
      DO 6007  INT = 1,NT
         IPO = IPO + 15
         CALL CORE(HWORK,64)
         WRITE(JUSCRN,6004) IDATA(IPO+1),(ADATA(IPO+1+JJ),JJ=1,8)
 6004    FORMAT(' ',I3,F8.2,2F7.3,3F8.2,2F7.3)
         CALL SYSSYM(XS,YS,SIZE,HWORK,64,0.)
         YS = YS - DEL
         CALL CORE(HWORK,42)
         WRITE(JUSCRN,6005) (ADATA(IPO+1+JJ),JJ=9,11),
...
C--------------- SPECIAL FORMAT FOR DEDX BANKS
 ...
 7001 FORMAT(' ',I3)
      ...
      DO 7004  ITR = 1,NTR
         IPO = LIM2 + 1 + (ITR-1)*10
         CALL CORE(HWORK,92)
         WRITE(JUSCRN,7002) IDATA(IPO+1),(ADATA(IPO+1+JJ),JJ=1,6),
...
C--------------- SPECIAL FORMAT FOR LGCL BANKS
 9000 LIM2 = LIM2 - 1
      CALL CORE(HWORK,29)
      WRITE(JUSCRN,9001) (IDATA(LIM2+LL),LL=1,4)
 9001 FORMAT(' ',4I7)
      CALL SYSSYM(XS,YS,SIZE,HWORK,29,0.)
      LIM2 = LIM2 + 4
      YS   = YS - DEL
      CALL CORE(HWORK,118)
      WRITE(JUSCRN,9002) (IDATA(LIM2+LL),LL=1,6),(ADATA(LIM2+LL),LL=7,10
 ...
 9002 FORMAT(' ',I3,I9,4I4,4F8.3,I4,4F8.3,2I3,I6,3I3)
      ...
      YS     = YS - DEL
      CALL CORE(HWORK,101)
      WRITE(JUSCRN,9003) ((HDATA(LIM22+L),HDATA(LIM22+L+1)),L=1,NCLS2,2)
 9003 FORMAT(' ',20I5)
      ...
      DO 9006  ITR = 1,NCLST
         ...
         CALL CORE(HWORK,119)
         WRITE(JUSCRN,9004) IDATA(IPO+1),(ADATA(IPO+1+JJ),JJ=1,6),
...
10000 NTR  = IDATA(LIM2)
      LIM2 = LIM2 - 1
      CALL CORE(HWORK,17)
      WRITE(JUSCRN,10001) (IDATA(LIM2+I),I=1,4)
10001 FORMAT(' ',4I4)
      ...
      DO 10004  ITR = 1,NTR
         IPO = LIM2 + 4 + (ITR-1)*14
         CALL CORE(HWORK,118)
         WRITE(JUSCRN,10002) (IDATA(IPO+I),I=1,3),(ADATA(IPO+I),I=4,14)
...
15001 LO   = IDATA(LIM2+1)
      ...
      IPO  = LIM2 + LO
      CALL CORE(HWORK,106)
      WRITE(JUSCRN,15002) (IDATA(LIM2+II),II=1,LO)
15002 FORMAT('  ',6I8,2F8.4,5I8)
      ...
      IF( LO1 .LT. 7 ) GO TO 15004
         YS = YS - DEL
         CALL CORE(HWORK,124)
         WRITE(JUSCRN,15003) (ADATA(LIM2+II),II=7,LO1)
...
16003    CONTINUE
         IILIM = 6*ILIM + 1
         CALL CORE(HWORK,IILIM)
         WRITE(JUSCRN,16004) (HWORK(LLO+100),LLO=1,ILIM)
16004    FORMAT(' ',20I6)
         CALL SYSSYM(XS,YS,SIZE,HWORK,IILIM,0.)
16005 CONTINUE
      GO TO 90100
...
19000 LIM2 = LIM2 - 1
      ...
      YS   = YS - DEL
      CALL CORE(HWORK,28)
      WRITE(JUSCRN,19001) HDATA(LH2+3)
...
C TRIGGERS
      ...
      IF( NTRI .LE. 0 ) GO TO 19003
         YS = YS - DEL
         CALL CORE(HWORK,77)
         WRITE(JUSCRN,19002) (HDATA(LH2+NTRI+K),K=1,4)
...
19003 NCOU = HDATA(LH2+5)
      IF( NCOU .LE. 0 ) GO TO 19005
         ...
         YS = YS - DEL
         CALL CORE(HWORK,53)
         WRITE(JUSCRN,19004) HDATA(LH2+NCOU+1),AA,HDATA(LH2+NCOU+3),
     &                       HDATA(LH2+NCOU+4)
...
19005 NL = HDATA(LH2+6)
      IF( NL .LE. 0 ) GO TO 19007
         ...
         YS  = YS - DEL
         CALL CORE(HWORK,124)
         WRITE(JUSCRN,19006) (HDATA(LH2+NL+K),K=1,4),AA1,AA2,AA3,AA4,
...
19009 CONTINUE
      YS = YS - DEL
      CALL CORE(HWORK,46)
      WRITE(JUSCRN,19010) (HDATA(LH2+ND+K),K=1,3)
19010 FORMAT(' ID ZVTX FLAG ',I3,'  ZVTX IN MM ',I6,' INRGZV ',I2)
      ...
      NTRT   = HDATA(LH2+ND+5)
      CALL CORE(HWORK,49)
      IF( NTR .LT. 0 ) WRITE(JUSCRN,19011)
19011 FORMAT(' PATTERN RECOGNITION NOT PERFORMED              ')
      IF( NTR .EQ. 0 ) WRITE(JUSCRN,19012)
...
19013    FORMAT(' PATREC PERFORMED,',I3,' TRACKS FOUND',I3,
     &          ' TRACK BANKS')
         ...
         YS   = YS - DEL
         CALL CORE(HWORK,102)
         WRITE(JUSCRN,19014) NTRT,LORT,LTRT,MWR1,MWR2,MWR3,IEFA
...
19015       CONTINUE
            CALL CORE(HWORK,120)
            WRITE(JUSCRN,19016) ITR,IFTR,RR1,FI1,
...
C
            YS = YS - DEL
            CALL CORE(HWORK,115)
            WRITE(JUSCRN,19017) TANTH,Z0,SIGZ,NZ
...
19019 CONTINUE
      CALL CORE(HWORK,44)
      WRITE(JUSCRN,19020) IPLT1,IPLT2,IPLT3,IPLT4
...
19022    CONTINUE
         ...
         WRITE(JUSCRN,19023) (HDATA(II),II=IPLT1,IPLT22)
...
19026    CONTINUE
         CALL CORE(HWORK,IRESV)
         WRITE(JUSCRN,19027) (HDATA(II),II=IPLT2,IPLT33)
...
19030    CONTINUE
         CALL CORE(HWORK,IRESV)
         WRITE(JUSCRN,19031) (HDATA(II),II=IPLT3,IPLT44)
...
19033 NM = HDATA(LH2+8)
      IF( NM .LE. 0 ) GO TO 19036
         ...
         NMU   = 2*NMU + 1
         CALL CORE(HWORK,64)
         WRITE(JUSCRN,19034) HDATA(LH2+NM),HDATA(LH2+NM+1),
...
19034    FORMAT(' NR MUON TRACKS ',I2,'   IN BARREL ',I3,
     &          ' POSITION ',5I4)
         ...
         YS = YS - DEL
         CALL CORE(HWORK,64)
         WRITE(JUSCRN,19035) (HDATA(LH2+NM+K+1),K = 2,NMU,2)
19035    FORMAT('                ',3X,'             ',3X,
     &          ' QUALITY ',5I4)
         CALL SYSSYM(XS,YS,SIZE,HWORK,NCORE,0.)
C
19036 CONTINUE
      ...
      IF( NTG .LE. 0 ) GO TO 19038
         YS = YS - DEL
         CALL CORE(HWORK,50)
         WRITE(JUSCRN,19037) (HDATA(LH2+NTG+K),K=1,2)
...
20002    CONTINUE
         IILIM = 108
         CALL CORE(HWORK,IILIM)
         WRITE(JUSCRN,20003) IZWRD1,IBT151,KANAL1,IFIFF1,HITCN1,IUNDF1,
     &                       IOVRF1,IZWRT1,
...
C ONLY DATA WORDS
     ...
     &   WRITE(JUSCRN,20103) IZWRD1,IBT151,IOR1,IUL1,ILL1,
     &                       IOVRF1,IZWRT1,
...
C  DATA + ADRESS
     ...
     &   WRITE(JUSCRN,20109) IZWRD1,IBT151,IOR1,IUL1,ILL1,
     &                       IOVRF1,IZWRT1,
...
C   ADRESS + DATA
     ...
     &   WRITE(JUSCRN,20110) IZWRD1,IBT151,ICRA1,ISLO1,ISUB1,
     &                       IZWRD2,IBT152,IOR2,IUL2,ILL2,
     &                       IOVRF2,IZWRT2
...
C  ONLY ADRESS WORDS
     ...
     &   WRITE(JUSCRN,20111) IZWRD1,IBT151,ICRA1,ISLO1,ISUB1,
     &                       IZWRD2,IBT152,ICRA2,ISLO2,ISUB2
...
C
         CALL CORE(HWORK,66)
         WRITE(JUSCRN,22004) (HDATA(IAPOIN+I),I=1,6), IMPROC
...
22007       DO 22008 I = 1, IEND
               ILB = I*2
               IHB = ILB - 1
               HBYTE(IHB) = LAND( HDATA( IPMPST+2+I ), 65280 )/256
...
C
            IF( ILSTR .GT. 37 ) GO TO 22010
               ...
               WRITE(JUSCRN,22009) ILWSDB, IWIRSD, (HBYTE(I),I=1,ILSTR)
22009          FORMAT(' ',I4,I4,'   ',37(Z2,1X))
               CALL SYSSYM(XS,YS,SIZE,HWORK,ICORE,0.)
               GO TO 22020
C
22010          CALL CORE(HWORK,123)
               WRITE(JUSCRN,22009) ILWSDB, IWIRSD, (HBYTE(I),I=1,37)
...
C
               ...
               ICORE = 12 + 3*ILCON
               CALL CORE(HWORK,ICORE)
               WRITE(JUSCRN,22011) (HBYTE(37+I),I=1,ILCON)
...
23000 LIM2   = LIM2 - 1
      ...
      LOLLOL = 10
      CALL CORE(HWORK,41)
      WRITE(JUSCRN,23001) (IDATA(LIM2 + I),I=1,LOL)
23001 FORMAT(' ',5I8)
...
23004    CONTINUE
         IILIM = 6*ILIM + 1
         CALL CORE(HWORK,IILIM)
         WRITE(JUSCRN,504) (HWORK(100+I),I=1,ILIM)
         CALL SYSSYM(XS,YS,SIZE,HWORK,IILIM,0.)
23005 CONTINUE
      GO TO 90100
...
24000 LIM2   = LIM2 - 1
      ...
      YS     = YS - DEL
      CALL CORE(HWORK,44)
      WRITE(JUSCRN,24001) (HDATA(LIMLIM + I),I=1,LOLLOL)
24001 FORMAT(' ',I3,5I8)
      CALL SYSSYM(XS,YS,SIZE,HWORK,44,0.)
      LENGTH = 2*IDATA(LIM2)
      LENGB  = LENGTH - LOLLOL
...
24003    LIML = LIMLIM + 8*(LHO - 1) + LOLLOL
         ...
         IF( ILIM .LE. 0 ) GO TO 90100
         CALL CORE(HWORK,50)
         WRITE(JUSCRN,24004) (HDATA(LIML+I),I=1,ILIM)
24004    FORMAT('  ',8I6)
         CALL SYSSYM(XS,YS,SIZE,HWORK,50,0.)
24005 CONTINUE
      GO TO 90100
...
25000 LIM2   = LIM2 - 1
      ...
      YS     = YS - DEL
      CALL CORE(HWORK,44)
      WRITE(JUSCRN,25001) (HDATA(LIMLIM + I),I=1,LOLLOL)
25001 FORMAT(' ',I3,5I8)
      CALL SYSSYM(XS,YS,SIZE,HWORK,44,0.)
      LENGTH = 2*IDATA(LIM2)
      LENGB = LENGTH - LOLLOL
...
25003    LIML = LIMLIM + 12*(LHO - 1) + LOLLOL
         ...
         IF( ILIM .LE. 0 ) GO TO 90100
         CALL CORE(HWORK,74)
         WRITE(JUSCRN,25004) (HDATA(LIML+I),I=1,ILIM)
25004    FORMAT('  ',12I6)
...
26000 LIM2   = LIM2 - 1
      ...
      YS     = YS - DEL
      CALL CORE(HWORK,44)
      WRITE(JUSCRN,26001) (HDATA(LIMLIM + I),I=1,LOLLOL)
26001 FORMAT(' ',I3,5I8)
      CALL SYSSYM(XS,YS,SIZE,HWORK,44,0.)
      LENGTH = 2*IDATA(LIM2)
...
26003 ILENWS = HDATA(IPARRY+1)
      IF( ILENWS .GT. 24 ) GO TO 26005
         ICORE = 2 + ILENWS*5
         CALL CORE(HWORK,ICORE)
         WRITE(JUSCRN,26004) (HDATA(IPARRY+I),I=1,ILENWS)
26004    FORMAT(' ',I4,I5,2X,22I5)
         CALL SYSSYM(XS,YS,SIZE,HWORK,ICORE,0.)
         GO TO 26007
26005 CALL CORE(HWORK,122)
      WRITE(JUSCRN,26004) (HDATA(IPARRY+I),I=1,24)
      CALL SYSSYM(XS,YS,SIZE,HWORK,122,0.)
C
      YS = YS - DEL
      ILCON = ILENWS - 24
      ICORE = 12 + 5*ILCON
      CALL CORE(HWORK,ICORE)
      WRITE(JUSCRN,26006) (HDATA(IPARRY+24+I),I=1,ILCON)
...
28000 LIM2  = LIM2 - 1
      ...
      IPO   = LIM2 + LO
      CALL CORE(HWORK,79)
      WRITE(JUSCRN,28001) (IDATA(LIM2+II),II=1,6),(ADATA(LIM2+JJ),JJ=7,8
     &),IDATA(LIM2+9)
