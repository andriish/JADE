C   02/09/80 311261723  MEMBER NAME  ZSLINE   (S)           FORTRAN
...
C        SELECT HIT TRIPLES
      ...
      DZ2 = Z(J) - Z(J+2)
      IF(ABS(DZ1).GT.140.) GOTO 15
      IF(ABS(DZ2).GT.100.) GOTO 15
      NTRIP = NTRIP + 1
      JTRIP(NTRIP) = J
...
C        BAD FIT, REMOVE WORST TRIPLE
      ...
      JMAX = J
   22 CONTINUE
      IF(NMULT.EQ.1 .AND. SIGMAX.LT.100.) GOTO 23
      JTRIP(JMAX) = 0
      GOTO 20
