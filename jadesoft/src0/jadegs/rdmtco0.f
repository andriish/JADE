C   01/11/84 712141934  MEMBER NAME  RDMTCO0  (JADEGS)      FORTRAN
C
C-----------------------------------------------------------------------
...
C   LAST MOD:   E. ELSEN    11/12/87 :  FOR NEW SMEARING SCHEMES IN
...
C
      COMMON / CADMIN / IEVTP,DUMM(6),ISMEAR,IJETCI
...
6110  FORMAT(' ---> RDMTCO: VERSION FROM  DEC 1987')
C
...
C
  150 CALL BDLS( 'MTCO', 1)
      CALL BCRE( IPMTCO, 'MTCO', 1, 156, &9090, IER )
C
...
C
C  WORD3 :   BINDL8 AND RJITT SMEARING VARIABLES ARE STORED, IF ISMEAR=100053000
C
