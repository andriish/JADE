C   10/05/82 608291510  MEMBER NAME  RDTRG2   (JADEGS)      FORTRAN
      SUBROUTINE RDTRG2
C***********************************************************************
...
C
 ...
 303         CONTINUE
             HWORD(KWD)=LOR(HWORD(KWD),MASK(KBIT))
 302         CONTINUE
C
...
C
             ...
             KWD=(I-1)/16*4+3
             KBIT=I-(I-1)/16*16
             IF(HJTRKA(I).EQ.1) HWORD(KWD)=LOR(HWORD(KWD),MASK(KBIT))
             IF(HJTRKF(I).EQ.1)HWORD(KWD+1)=LOR(HWORD(KWD+1),MASK(KBIT))
  600        CONTINUE
C
...
C                          T2ACC,T2POST,  T3ACC
      ...
      HDATA(K) = 0
      DO 1101 IBIT=1,8
      IF(IW22(IBIT).GE.1) HDATA(K)=LOR(HDATA(K),MASK(IBIT))
 1101 CONTINUE
C                                           FILL BEATES SPECIAL WORD
C                                           IN TRIG,1 SHOWING ACTUAL
C                                           T2 ACCEPT SOURCE
      HDATA(IND1*2+9) = LOR(HDATA(IND1*2+9),ICO1)
C
      RETURN
C
...
C                                           IN TRIG,1 SHOWING ACTUAL
C                                           T2 ACCEPT SOURCE
      HDATA(IND1*2+10) = LOR(HDATA(IND1*2+10),ICO1)
