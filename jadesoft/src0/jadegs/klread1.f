C   20/12/83 512191350  MEMBER NAME  KLREAD   (JADEGS)      FORTRAN
C
C-----------------------------------------------------------------------
      SUBROUTINE KLREAD
C-----------------------------------------------------------------------
C
C
C        NEW VERSION:                    21. 9. 82      P. STEFFEN
C
...
C
C        USE COMMON /CMCCAL/ LBMC(15)  TO INDICATE IF
...
C
      ...
      DIMENSION NAME(15),LENG(15),MAXL(15)
C
      DATA NBANKS /12/
      DATA NAME/'MUCA','LGMA','TAGS','JTPL','JTAB',
     1          'TOFC','LGST','DEDX','SPTG','RVTX',
     2          'ZCAL','TAGF','    ','    ','    '/
      DATA LENG /2,2,2,2,4,  4,2,2,2,4,  4,2,0,0,0/
...
C
 2900 FORMAT(' FIRST CALL TO KLREAD, EVENT DATE: ',6I5,
     *             ' TZERO:',3F6.2)
 2901 FORMAT('0KLREAD WARNING : NO "SPINNING" BLOCK DATA, RECALIBR. OF L
     ,EADGLASS NOT POSSIBLE')
...
C     SKIP THE FRUTHER READING OF THE FILES.
C
      ILBMCS = 0
      DO 1550  ILB = 1,15
