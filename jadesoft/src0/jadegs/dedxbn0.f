C   06/03/82 711301713  MEMBER NAME  DEDXBN0  (JADEGS)      FORTRAN77
      SUBROUTINE DEDXBN
C-----------------------  LAST CHANGE: 20.11.87   E.E.      ---C
...
C
C  NHFCUT AND PMCUT ARE SET IN JADEBD TO THESE VALUES
C  REMOVED FROM HERE IN ORDER TO ALLOW THE USER TO SET THEM REMOTELY
C                                  J.O.  16.11.87
C     NHFCUT = 10
C     PMCUT  = 0.01
C-
      IF(ICALL.EQ.1 .AND. NDDINN .EQ. 0 ) PRINT 911, NHFCUT,PMCUT
  911 FORMAT('  -----------       ( LAST CHANGE 10/11/87 )       '/
...
C
              ...
              IF (ILAY.GT.0 .AND. ILAY.LE.16) THEN
                IHDT(ILAY) = NHIT
                IZB(NHIT) = 0
...
C ------------------------------------------ REMOVE HITS IN CELL CORNERS
         ...
         IF(NR.EQ.2.AND.NL.EQ.14.AND.LRFL(J).EQ.1.AND.T.GT.180.) GOTO 2600037500
