C   15/03/79 808021801  MEMBER NAME  LGCLPB9  (SOURCE)      FORTRAN
C
C--- RESET TO VERSION BEFORE 12/87 ( CALIBARATION CORRECTION IS
C     DONE IN CALCOR, CALLED FROM LGANAL, NOW )  02.08.88 D.PITZL
C
C*************************************************************
      SUBROUTINE LGCLPB
C*************************************************************
...
C
      ICALLS = ICALLS + 1
      IF ( ICALLS .EQ. 1 ) PRINT 2732
 2732 FORMAT ( T2,'JADELG.LOAD (LGCLPB9) CALLED FROM LGANAL.',
     +   ' RESET VERSION OF 02.08.88' )
C
C
      ZSTEP=ZWID(1)+ZGAP(1)
