C   12/05/80 311111120  MEMBER NAME  GPHNAR1  (S)           FORTRAN
      IMPLICIT INTEGER*2 (H)
C---
C---   SUMMER 1982 VERSION
C---
...
#include "ciouni.for"
      ...
      DIMENSION IDATAL(11),HVOLSR(4),HELP(10),IMUC(11,4)
...
C---
   ...
   16 CONTINUE
    ...
    3 HVOLSR(I)=HBLANK
      ...
      WRITE(6,831) HERR,HERR
831   FORMAT(' ERROR IN GETPDD: HERR = ',I8,1X,Z4)
C---
...
62    LSTPS(I)=0
      ...
      SSTPS(6)=.TRUE.
C---
      CALL TRMOUT(80,'DEFAULT STANDARD VIEW "RB" AND USER LEVELS 2 AND 600012800
     $ HAVE BEEN SET.    ^')
     ...
     $')
      CALL TRMOUT(80,'  ^')
C---
C     CALL TRMOUT(80,'NAME OF  OUTPUT DATA SET / EDIT BACKUP FILE  WILL
C    $BE ASKED FOR ^')
C     CALL TRMOUT(80,'AT THE FIRST COMMAND  "WRIT" OR "EDIT (SAVE)"^')
C     CALL TRMOUT(80,'  ^')
C---
...
8011  CONTINUE
  ...
  893 HVOLSR(I)=HBLANK
      ...
      LUNITA(1)=LUNNNN
      GO TO 801
C---     ERROR HAS OCCURRED ON ALLOCATION OF AUPDAT1 FILE
  795 CALL TRMOUT(80,'ERROR IN ALLOCATING F11LHO.AUPDAT1.  THE BUPDAT FI
     $LES WILL BE TRIED..^')
      IF(NCALI.EQ.1) GO TO 8033
775   CALL TRMOUT(80,'ALLOCATION FAILURE FOR FOR A AND B FILES. PROGRAMM
     $ED STOP...^')
      STOP
C---     ERROR HAS OCCURRED ON ALLOCATION OF BUPDAT0/1 FILE
  785 CALL TRMOUT(80,'ERROR IN ALLOCATING BUPDAT0,1 FILES.  THE AUPDAT1
     $FILE WILL BE TRIED..^')
      IF(NCALI.EQ.2) GO TO 801
      GO TO 775
803   NCALI = 2
8033  CONTINUE
      DO 797  II = 1,2
      DO 798 I=1,11
      IDATAL(I)=IMUC(I,II)
      IF(II.EQ.2.AND.HDUM.EQ.HCAL) IDATAL(I)=IMUC(I,4)
798   CONTINUE
      DO 793 I=1,4
  793 HVOLSR(I)=HBLANK
      LUNNNN=0
      CALL GETPDD(IDATAL,HVOLSR,DDN,LUNNNN,HERR)
      IF(HERR.NE.0) GO TO 785
      LUNITA(II)=LUNNNN
  797 CONTINUE
...
#include "chsym.for"
     ...
     $'O ','P ','Q ','R ','S ','T ','U ','V ','W ','X ','Y ','Z '/
      ...
      DATA DSPDTM /7*.FALSE.,.TRUE.,22*.FALSE./,ISTVW/0/,JTVW/0/
      COMMON/CVX/NNPATR,ICRSTR,NNJETC
      DATA NNPATR/0/, NNJETC/0/
C
