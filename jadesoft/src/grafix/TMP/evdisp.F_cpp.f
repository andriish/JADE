C   01/11/84 808091448  MEMBER NAME  EVDISP   (S)           FORTRAN
C
C-----------------------------------------------------------------------
...
C      MOD: J. HAGEMANN  27/02/87 : FOR 1986 ID-DATA SMALLER CROSSES IN
C                                   R-FI-VIEWS
C LAST MOD: J. HAGEMANN  09/08/88 : FOR VERTEX CHAMBER VIEW
C
C-----------------------------------------------------------------------
C
...
C
 ...
 5003 CONTINUE
C
      call setcol(' ') ! PMF 24/11/99: set colour
...
C
            IF( IDATA(IBLN('VTXC')) .LE. 0 ) GO TO 7051
            IF( .NOT. FLVCDO(4) ) GO TO 7070
C
C                                      FORCED DISPLAY OF  B P C H
 7051          ILBPCH = IDATA(IPBPCH)
C
...
C
      CALL RZHITS(IPZETC,XZ,YZ,XZ1,YZ1,XZ2,YZ2,ZZ1,ZZ2,IERR)
*** PMF 19/10/99      IF( IERR .EQ. 0 ) CALL RZDRAW(XZ,YZ,XZ1,YZ1,XZ2,YZ2,ZZ1,ZZ2,3)
*     Put variable NWIR as first argument in the parameter list of the CALL of RZDRAW,
*     according to the argument list expected in SUBROUTINE RZDRAW
      IF( IERR .EQ. 0 )
     +     CALL RZDRAW(NWIR,XZ,YZ,XZ1,YZ1,XZ2,YZ2,ZZ1,ZZ2,3)
*** PMF (end)
C
      IPZETC = IPZETC + 2
      GO TO 92
