C   16/02/84 711301733  MEMBER NAME  DEDXVW   (S)           FORTRAN
C
C-----------------------------------------------------------------------
      SUBROUTINE DEDXVW
C-----------------------------------------------------------------------
...
C        MOD: C. BOWDERY   21/03/84 :  BETTER MESSAGE IF NO TRACKS
C        MOD: J. OLSSON    23/08/84 :  NO CALL TO DEDXBN, DONE IN DEDXDS
C  LAST  MOD: J. OLSSON    01/09/87 :  UPDATE TO AMBRUS THEORY DESCRIP.
C
...
C
      CALL CORE(HMW,5)
      WRITE(cHMW,245) ! PMF 17/11/99: JUSCRN changed to cHMW
245   FORMAT(' 0.05')
      CALL EOUTST(5,HMW)
C
501   FRAC1 = (1.0 - FRAC) * 15.0
      ...
      CALL CORE(HMW,4)
      WRITE(cHMW,247) ! PMF 17/11/99: JUSCRN changed to cHMW
247   FORMAT(' 0.1')
      CALL EOUTST(4,HMW)
C
502   CALL MOVEA(FRAC1 + FRAC * 15.0, 0.0)
      ...
      CALL CORE(HMW,2)
      WRITE(cHMW,228) ! PMF 17/11/99: JUSCRN changed to cHMW
228   FORMAT(' 1')
      CALL EOUTST(2,HMW)
C
503   CALL MOVEA(FRAC1 + (1.0 + FRAC) * 15.0, 0.0)
      ...
      CALL CORE(HMW,3)
      WRITE(cHMW,229) ! PMF 17/11/99: JUSCRN changed to cHMW
229   FORMAT(' 10')
      CALL EOUTST(3,HMW)
C
504   CALL MOVEA(FRAC1 + (2.0 + FRAC) * 15.0, 0.0)
      ...
      CALL CORE(HMW,4)
      WRITE(cHMW,230) ! PMF 17/11/99: JUSCRN changed to cHMW
230   FORMAT(' 100')
      CALL EOUTST(4,HMW)
C
...
C
      NTRR = MIN0(NTR,60)
      IF( NTRR .NE. 0 .AND. IER.EQ.0 ) GO TO 3201
...
C
        ...
        IF(PDEDX .GT. 100 .OR. PDEDX .LT. PMCUT) GO TO 3202
C
        PDEDX  = ALOG10(PDEDX)
...
C
C      CALL DRTHCV( FRAC1, LARGE, SMALL )
C
C
C                            DRAW "THEORETICAL" CURVES FROM K. AMBRUS
C
       CALL DTHCV1( FRAC1, LARGE, SMALL )
C
...
C
      CALL MOVEA(22.,-2.5)
      CALL CORE(HMW,9)
      WRITE(cHMW,3210) ! PMF 17/11/99: JUSCRN changed to cHMW
3210  FORMAT(' Momentum')
      CALL EOUTST(9,HMW)
C
      CALL MOVEA(-6.,22.5)
      CALL CORE(HMW,7)
      WRITE(cHMW,3211) ! PMF 17/11/99: JUSCRN changed to cHMW
3211  FORMAT(' keV/cm')
      CALL EOUTST(7,HMW)
C
      CALL MOVEA(-6.,12.5)
      CALL CORE(HMW,6)
      WRITE(cHMW,3212) ! PMF 17/11/99: JUSCRN changed to cHMW
3212  FORMAT(' dE/dx')
      CALL EOUTST(6,HMW)
C***
C***                    WRITE CAPTION
