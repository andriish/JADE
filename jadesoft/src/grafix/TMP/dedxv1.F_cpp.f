C   16/02/84 711201808  MEMBER NAME  DEDXV1   (S)           FORTRAN
C
C-----------------------------------------------------------------------
      SUBROUTINE DEDXV1
C-----------------------------------------------------------------------
...
C        FIND MAX AND MIN (NON-ZERO) ENERGY DEPOSIT VALUE, AND X-SCALE
 ...
 2801 CONTINUE
      IF(DEDX.LT.EMIN)EMIN=DEDX
      IF(DEDX.GT.EMAX)EMAX=DEDX
...
C        TICK LABELS
      ...
      IXCURR=XCURR
      CALL CORE(HMW,5)
      WRITE(cHMW,245)IXCURR   ! PMF 17/11/99: JUSCRN changed to cHMW
245   FORMAT(' ',I4)
      CALL EOUTST(5,HMW)
C
C        TICK MARKS
...
C        TICK LABELS
      ...
      CALL CORE(HMW,4)
      WRITE(cHMW,246)IYCURR ! PMF 17/11/99: JUSCRN changed to cHMW
246   FORMAT(' ',I3)
      CALL EOUTST(4,HMW)
C        TICK MARKS
504   CONTINUE
...
514   CONTINUE
      ...
      CALL DSORTO(NPC,ARR,TRCM,DTRCM)
      XCURR=XVMAX+30.*IR
      YCURR=TRCM
...
C
      ...
      CALL CORE(HMW,9)
      WRITE(cHMW,3214) ! PMF 17/11/99: JUSCRN changed to cHMW
3214  FORMAT('Trc. mean')
      CALL EOUTST(9,HMW)
      CALL MOVEA(XVMAX+15.,YVMIN-4.5*SSY)
      CALL CORE(HMW,9)
      WRITE(cHMW,3215) ! PMF 17/11/99: JUSCRN changed to cHMW
...
C
      ...
      CALL CORE(HMW,8)
      WRITE(cHMW,3212)NN ! PMF 17/11/99: JUSCRN changed to cHMW
3212  FORMAT(' Track',I2)
      CALL EOUTST(8,HMW)
      GO TO 519
 518  CALL MOVEA(AMAX1(XVMIN-9.5*SSX,XMIN),YVMAX+0.5*SSY)
      CALL CORE(HMW,9)
      WRITE(cHMW,3213)NN ! PMF 17/11/99: JUSCRN changed to cHMW
3213  FORMAT(' Track',I3)
      CALL EOUTST(9,HMW)
 519  CONTINUE
      IF( HEAD(18) .LE.100 ) GO TO 520
        CALL MOVEA(XVMIN+0.5*(XVMAX-XVMIN)-7.*SSX,YVMAX+0.5*SSY)
        CALL CORE(HMW,14)
        WRITE(cHMW,3216) ! PMF 17/11/99: JUSCRN changed to cHMW
 3216   FORMAT('dE/dx per wire')
        CALL EOUTST(14,HMW)
      GO TO 521
  520 CONTINUE
        CALL MOVEA(XVMIN+0.5*(XVMAX-XVMIN)-13.*SSX,YVMAX+0.5*SSY)
        CALL CORE(HMW,26)
        WRITE(cHMW,3217) ! PMF 17/11/99: JUSCRN changed to cHMW
 3217   FORMAT('Theoretical dE/dx per wire')
        CALL EOUTST(26,HMW)
  521 CONTINUE
C***
C***                    WRITE CAPTION
