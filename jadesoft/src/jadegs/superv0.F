C   01/04/85 504150547  MEMBER NAME  SUPERV   (JADEGS)      FORTRAN
C
C-----------------------------------------------------------------------
      SUBROUTINE SUPERV
C-----------------------------------------------------------------------
...
C   LAST MOD  C. BOWDERY   1/04/85 :  CALL KALIBR FOR EVERY EVENT NOW
C
C
C------------------  M O D E   O F   U S E  ----------------------------
C
...
C
  ...
  600 FORMAT( /1X,33('****')/
     ...
     +       ' *',32('    '),'  *'/
     +       ' *',3(8X,'  V E R S I O N     1/04/85  ',5X),4X,'*'/
     +       ' *',32('    '),'  *'/
     +       ' *',32('    '),'  *'/
     +         1X,33('****')  //       )
C
...
C                            COMMAND HAS JUST BEEN GIVEN (LSTCMD=112)
C
      IF(   NDDINN .NE. 0         .AND.
     +    ( NDDOUT .EQ. 0  .OR.  LSTCMD .NE. 112 ) ) GO TO 1
