#!/usr/bin/ksh
# Job to generate 4-vector MC with HERWIG59
# Modifications:
...
+PAM,12,R=HWCMDS,T=A,C. herwig59.car
+PAM,13,T=A,C. mc$VERSION.car
+QUIT
EOF

# Run patchy to get the code:
ytofort -D mchw59_$VERSION.cra
rc=$?
if [ $rc -ne 0 ] ; then
     echo 'hw59_qq: patchy return code' $rc
     echo `whoami` -m 'hw59_qq: patchy wrong'
     exit $rc
fi
# Compile, output goes in subdirectory:
fort mchw59_$VERSION.f
rc=$?
if [ $rc -ne 0 ] ; then
     echo 'hw59_qq: compilation return code' $rc
     echo `whoami` -m 'hw59_qq: compilation wrong'
     exit $rc
fi
...
link -m -D $ulink mchw59_$VERSION.o \
	libmc$VERSION libherwig59 \
	libjt74opal libckern105 libpx114
rc=$?
if [ $rc -ne 0 ] ; then
  echo 'hw59_qq: bad news, link return code' $rc
  echo `whoami` -m 'mc-comp_link: no executable made'
  exit $rc
fi

mkdir -p ${JADE_MCBIN}
if [ -x mchw59_$VERSION.run ]; then
    mv -f mchw59_$VERSION.run ${JADE_MCBIN}/.
    echo "mchw59_$VERSION.run moved to ${JADE_MCBIN}/."
else
...
rm -f mchw59_$VERSION.cra
rm -f mchw59_$VERSION.lis
#rm -f mchw59_$VERSION.f
rm -f mchw59_$VERSION.o
exit
...
# Execute:
time ${JADE_MCBIN}/mchw59_$VERSION.run << EOF
LIST
C
