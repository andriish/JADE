#!/usr/bin/ksh
# Job to generate 4-vector MC with PYTHIA 5.721
# Modifications:
...
# Set the name of the .hist, .cprod (and .evki) file for this run:
NAME=py57
...
+PAM,14,T=A,C. pymaxi.car
+QUIT.
EOF

# Run patchy to get the code:
ytofort mcpy57_$VERSION
rc=$?
if [ $rc -ne 0 ] ; then
     echo 'py57_qq: patchy return code' $rc
     echo `whoami` -m 'py57_qq: patchy wrong'
     exit $rc
fi
# Compile, output goes in subdirectory:
fort mcpy57_$VERSION.f
rc=$?
if [ $rc -ne 0 ] ; then
     echo 'py57_qq: compilation return code' $rc
     echo `whoami` -m 'py57_qq: compilation wrong'
     exit $rc
fi
...
link -m -D $ulink mcpy57_$VERSION.o  \
	libmc$VERSION libpythia5722 \
	libjt74opal libckern105 libpx114
rc=$?
if [ $rc -ne 0 ] ; then
  echo 'py57_qq: bad news, link return code' $rc
  echo `whoami` -m 'mc-comp_link: no executable made'
  exit $rc
fi

mkdir -p ${JADE_MCBIN}
if [ -x mcpy57_$VERSION.run ]; then
    mv -f mcpy57_$VERSION.run ${JADE_MCBIN}/.
    echo "mcpy57_$VERSION.run moved to ${JADE_MCBIN}/."
else
...
rm -f mcpy57_$VERSION.cra
rm -f mcpy57_$VERSION.lis
#rm -f mcpy57_$VERSION.f
rm -f mcpy57_$VERSION.o
exit
...
# Execute:
time ${JADE_MCBIN}/mcpy57_$VERSION.run << EOF
LIST
C
