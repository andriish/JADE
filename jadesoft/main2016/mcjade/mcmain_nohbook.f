...
C
C PAW
CAV      INTEGER NWPAW,ICYCLE,HMEMOR
CAV      PARAMETER (NWPAW=2000000)
CAV      COMMON / PAWC / HMEMOR(NWPAW)
C
C FFREAD
      REAL SPACE
      INTEGER FFSPAC
...
CAV      IF(LHIST) THEN
CAV         WRITE(*,'(/,A,/,27(''-''))') 'MCMAIN: Control histograms:'
CAV         CALL MCEND
CAV         CALL HCDIR('//PAWC',' ')
CAV         CALL HRPUT(0,CHISTF(1:LENOCC(CHISTF)),'N')
CAV      ENDIF
C
C... Some info
