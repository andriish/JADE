C   09/02/89 903061120  MEMBER NAME  MCTAGMPL (S)           F77
C
...
C A.J.FINCH 13/3/84
C MODIFICATIONS : --
C  (C.HOWARTH - 14/2/89) INNER EDGE ACCEPTANCE  FOR MARKMC = 4
C                        TAGGER MODIFIED FROM 32 MRAD TO 19.4 MRAD TO
C                        ALLOW FOR POSSIBILITY OF BEAMPIPE INTERACTIONS.
C
C                        IF MARKMC=4, GEANT SIMULATION IS NOW CALLED
C                        INSTEAD OF SIMPLE SHOWER CALCULATION.
C
C           J.O. MODIFICATION: GEANT CALLED IF LFLAG(8) = .TRUE.
C           J.O. MODIFICATION: UPDATE PV-VECTOR IF TAGGER HIT
C
C=======================================================================
C
...
C    THMINP - +Z
C
      DATA THMINM/0.0335,0.0361,0.0410,7*0.0194/
      DATA THMINP/0.0335,0.0361,0.0410,7*0.0194/
...
C
      CALL HBOOK1(2201,'MCTAGM: ETOT BEFORE MCTAGE$',100,0.,50.)
      CALL HBOOK1(2202,'MCTAGM: ETOT AFTER MCTAGE$',100,0.,50.)
      ICALLP = 1
11    CONTINUE
C
C
...
C ======== CODE ======
C
C  MODIFY INNER ACCEPTANCE TO OLD VALUE, IF GEANT IS NOT USED
C
      IF(.NOT.LFLAG(8)) THMINM(4) = .032
      IF(.NOT.LFLAG(8)) THMINP(4) = .032
...
C
       CALL VZERO(AGG,200)
C
       IF(LFLAG(8).AND.MARKMC.GE.4) THEN
            CALL TGEANT (PV,R0)
            GOTO 2000
       ENDIF
         SUMPLO = 0.
