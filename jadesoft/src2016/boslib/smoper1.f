C   07/06/96 606071909  MEMBER NAME  SMOPER1  (S4)          FORTG1
...
C
C     (7) = FL
C     FOLD WITH WEIGTHS 1 4 8 12 14 12 8 4 1  /64
C
   80 NM=N-1
      IF(NM.LT.2) GOTO 10
      DO 82 I=1,N
   82 Y(I)=Y(I)/64.0
      ...
      KK=MOD(K+1,2)
      ZL=Y(1)+Y(2)
      IF(KK.EQ.0) ZL=ZL+Y(1)+Y(1)
      DO 84 I=2,NM
      ZN=Y(I-1)+Y(I+1)
      IF(KK.EQ.0) ZN=ZN+Y(I)+Y(I)
      Y(I-1)=ZL
   84 ZL=ZN
      ZN=Y(N-1)+Y(N)
      IF(KK.EQ.0) ZN=ZN+Y(N)+Y(N)
      Y(N-1)=ZL
   86 Y(N)=ZN
      GOTO 10
C
