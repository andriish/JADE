C   07/06/96 606071906  MEMBER NAME  RDVAR    (S4)          FORTG1
      ...
      INTEGER ARRAY(1),INI,NRD,NWR,NTOP
      DATA INI/0/, NDR/0/,NWR/0/
      COMMON/CIBCOM/IRFLG
***PMF      REAL*8 TEXTA(4)/' IBCOMR ',' IBWRD  ',' IBARR  '/
      CHARACTER*8 TEXTA(4)
      DATA TEXTA/8H IBCOMR ,8H IBWRD  ,8H IBARR  ,8H        /
      CHARACTER*8 TEXTB(3)
      DATA TEXTB/8H        ,8H IHN213 ,8H IHN218 /
      EXTERNAL IBCERR,BAEND
C
CAV      IF(INI.EQ.0) CALL IBCERR
C
      NRD=NRD+1
      IRFLG=1
      IRCDE=0
CAV      CALL IBCOMR(IUN,*8,*4)
CAV      CALL IBWORD(NTOT,*7,*3)
      IF(NTOT.LE.0.OR.NTOT.GT.NDIM) GOTO 1
CAV WHY     CALL IBARR(ARRAY,NTOT,*6,*2)
CAV WHY      CALL IBCOMF
      GOTO 100
C     RECORD TO LARGE OF FIRST WORD WRONG
    1 CALL IBCOMF
CAV      WRITE(6,101) IUN,NRD,NDIM,NTOT,NTOT,NTOT
      NTOT=0
      GOTO 100
C     READ ERRORS
    2 IRCDE=IRCDE+1
    ...
    5 CONTINUE ! WRITE(6,102) IUN,NRD,TEXTA(IRCDE),TEXTB(IRFLG)
      NTOT=0
      GOTO 100
C     END EXITS
    6 IRCDE=IRCDE+1
    7 IRCDE=IRCDE+1
    8 IRCDE=IRCDE+1
    9 CONTINUE !WRITE(6,103) IUN,NRD,TEXTA(IRCDE)
      NTOT=-1
      GOTO 100
C
CAV      ENTRY WRVAR(IUN,NTOT,ARRAY)
C
...
CAV      CALL IBCOMF
      GOTO 100
C
CAV      ENTRY WRFIX(IUN,NTOT,ARRAY)
C
      IF(INI.EQ.0) CALL IBCERR
C
      NWR=NWR+1
...
CAV      CALL IBCOMW(IUN)
   ...
   20 CONTINUE !WRITE(6,104) IUN,NWR
      CALL ABEND
C
  100 INI=1
      RETURN
C
  ...
  104 FORMAT('0---- UNIT',I3,'   WRITE ',I8,'.TH CALL',5X,'IO-ERROR',
     1   ' ---- ABEND'/)
      ...
      INTEGER INI
      DATA INI/0/
CAV      EXTERNAL ERR213,ERR218
      IF( INI .NE. 0) GOTO 100
CAV      CALL ERRSET(213,256,0,0,ERR213)
CAV      CALL ERRSET(218,256,0,0,ERR218)
      INI=1
  100 RETURN
      END
