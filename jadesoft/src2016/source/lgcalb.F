C   10/09/82            MEMBER NAME  LGCALB   (SOURCE)      FORTRAN
C   20/08/82 209101007  MEMBER NAME  LGCALB   (JADESR)      FORTRAN
      SUBROUTINE LGCALB(*)
C
C     S.YAMADA   09-03-79  10:50
C          MODIFICATI0N  14-11-79  01:35 BY Y.WATANABE
C          MODIFICATI0N  27-08-80  08:35 BY Y.WATANABE
C                INTERCHANGE 293 AND 613.
C          MODIFICATI0N  12-11-80  13:35 BY Y.WATANABERUNS=4865-5666
...
C
C     LAST MODIFICATION 03-09-82  13:55 BY S.YAMADA
C               CHECK OF EXISTENCE OF THE BAD CHANNEL LIST IN THE
C               CONSTANTS FILE.   LOADED  07-09-82   08:45
...
C     CALL CLOC( NP, 'ALGL',0 )
      ...
      IF(NP.LE.0) RETURN
C
C---- CHECK IF CALIBRATION DATA IS AVAILABLE
      IF( JCALFL.EQ.0 ) GO TO 105
C
C
...
C
C
C---- NO CALIBRATION DATA,  STOP THE JOB
  105 WRITE(6,605)
  605 FORMAT(//'0 LG CALIBRATION DATA IS NOT AVAILABLE.',
     1      /'0USE THE PROPER CALIBRATION CONST.FILE    <BUPDAT1>.',
     2    3(/'0 JOB IS STOPPED IN LGCALB.'))
      STOP
C
C---- WRONG POINTERS
110   WRITE(6,620)
