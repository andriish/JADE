C   15/03/79 712212046  MEMBER NAME  LGCLPB   (SOURCE)      FORTRAN
C
C
...
C
      ICALLS = ICALLS + 1
      IF ( ICALLS .EQ. 1  .AND.  NRUN .GT. 100 ) PRINT 2733
 2733 FORMAT (/T2,' JADELG.LOAD (LGCLPB) VERSION OF 22.12.87 CALLED',
     +    ' WITH CORRECTION FOR BHABHA LEAKAGE'/)

      IF ( ICALLS .EQ. 1  .AND.  NRUN .LT. 100 ) PRINT 2734
 2734 FORMAT (/T2,' JADELG.LOAD (LGCLPB) VERSION OF 22.12.87',
     +    ' CALLED FOR MC-DATA'/)
C
C
      ZSTEP=ZWID(1)+ZGAP(1)
...
C                                        LOOP OVER ALL BLOCKS IN CLUSTER
      ...
      DO 7082 NN = NS,NL
         W = HLGADC(2,NN)
         RBBLK = 0.0
         IF ( NRUN .GT. 100 )  CALL BBLEAK ( IZ, RBBLK )
         W = W * ( 1. - RBBLK )
         WSUM = WSUM + W
         HLGADC(2,NN) = W
 7082 CONTINUE
