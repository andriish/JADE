C   01/11/84 809101248  MEMBER NAME  RDMTCO9  (JADEGS)      FORTRAN
C
C-----------------------------------------------------------------------
...
6110  FORMAT(' ---> RDMTCO: VERSION FROM  JULY 1988')
C
...
C                      IN ORDER TO AVOID UNNECESSARY PRINT
      IF(NFLAGS(8).NE.0) CALL PRAGAI(IPRIT)
      NFLAGS(8) = 0
      IF(IPRIT.NE.0) NFLAGS(9) = 1
      IF(NFLAGS(9).NE.0)  WRITE(6,8010)
8010  FORMAT('  RDMTCO: FLAG9 SET TO WRITE CONSTANTS RECORDS ON OUTPUT')
...
C                      IN ORDER TO AVOID UNNECESSARY PRINT
      ...
      NFLAGS(8) = 0
      IF(IPRIT.NE.0) NFLAGS(9) = 1
C     IF(NFLAGS(9).NE.0)  WRITE(6,8011)
C011  FORMAT('  RDJETC: FLAG SET TO WRITE CONSTANTS RECORDS ON OUTPUT')
      IF(IPRIT.NE.0) CALL PRSTAT(IFLAG1,IFLAG2,IFLAG3)
C
