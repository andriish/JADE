C   21/08/87 711201630  MEMBER NAME  DXMCGN   (S)           FORTRAN77
      SUBROUTINE DXMCGN( PTOT, NHDEDX, IPART, NRUN,
     *                       DEDX, SDEDX, DEDXT )
C----------------------------------------------------------
C  Version of 21/08/87       Last Mod 27/08/87   E Elsen
C  Generate dE/dx and sigma(dE/dx) for momentum PTOT with
C  NHDEDX hits and JADE particle type IPART.
C
...
C           IPART = particle type = 1, 2, 3, 4, 5, 6 for
C                               gamma, e,mu,pi, K, p
...
C           DEDX  = dE/dx and
C           SDEDX =  error
C           DEDXT =  theoretical dEdx value
C----------------------------------------------------------
...
C----                                       USED IN K.AMBRUS' ROUTINES
      ...
      REAL*4 ASYS( 1:39)
      DATA ASYS/
     *   0.349,0.347,0.343,0.328,0.314,0.325,0.295,0.329,0.319,0.304,
     *   0.328,0.307,0.314,0.309,0.331,0.302,0.310,0.324,0.327,0.299,
     *   0.311,0.338,0.325,0.341,0.390,0.343,0.349,0.369,0.388,0.365,
     *   0.393,0.372,0.385,0.401,0.392,0.375,0.378,0.381,0.387/
      REAL*4 ADEDX(NMHITS)
C----                               -------
      REAL*4 AMASS(0:7) 
      DATA AMASS/ 2*.1396, .511E-3, .1057, .1396, .4937,
     *                     .9383, .1396 /
      LOGICAL FIRST 
      DATA FIRST / .TRUE. /
C
...
C
      IF( FIRST ) THEN
        FIRST = .FALSE.
C       CALL MCAMPE( FAMPLA, NAMPLA )
C       WRITE(6,9101) FAMPLA
C9101   FORMAT('  REAL*4 FAMPLA /',E12.4,'/')
C       DO 1 I=0,9
C         WRITE(6,9102) I,(NAMPLA(J),J=I*100+1,I*100+100), I, I
C9102     FORMAT('  INTEGER NAMPL',I1,' /'/9(' *',10(I5,',')/)
C    *               ' *',9(I5,','),I5,'/'/
C    *        '  EQUIVALENCE ( NAMPLA(',I1,'01),NAMPL',I1,'(1)) ')
C   1   CONTINUE
      ENDIF
C
      DEDX = 0.
...
C
C                                           PARTICLE MASS
      XM = AMASS(MAX(0,MIN(IPART,7)))
C
      DEDXT = DEDXTP( PTOT, XM, Z1 )
C                                           SYSTEMATIC ERROR
      ERRSYS = ASYS(IPCALF(NRUN))/SQRT(FLOAT(NDEDX))
      IF( ERRSYS .GT. 0. ) THEN
...
C                                           AMPLITUDES
      DO 10 IHIT = 1, NDEDX
        ...
        ADEDX(IHIT) = XDEDX/FAMPLA
   10 CONTINUE
C                                           TRUNCATED MEAN
      CALL DSORTO( NDEDX, ADEDX, TRCM, DTRCM )
      DEDX = DEDXT*Z*TRCM
      SDEDX = DEDXT*Z*DTRCM
      SDEDX = ERDEDX( DEDX, SDEDX, NRUN, NDEDX )
      END
