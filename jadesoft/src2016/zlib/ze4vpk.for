C   14/02/87 805311844  MEMBER NAME  ZE4VPK   (S)           FORTRAN77
...
C LAST MOD   31/05/88       E ELSEN     MODIFY MC PARTON PART
...
*** PMF 09/12/99      ENTRY ZE4VUP( NST, N, IGEN )
*** This ENTRY point is not called by any of the ZE4V procedures.
*** Comment this because of segmentation fault problems concerning the
*** variables N and NST !!! 
C--------------------------------------------------------
...
C
      ...
      IF ( NPHEAD .GT. 0 ) THEN
        NPH2 = 2 * NPHEAD
        HRUN(1) = HW( NPH2 + 6 )*100 + HW( NPH2 + 7 )
        HRUN(2) = HW( NPH2 + 8 )
        DATE = IRUN
        HRUN(1) = HW( NPH2 + 10 )
        HRUN(2) = HW( NPH2 + 11 )
...
C                                        FIND NO OF JETS
          NPMC = LH + NVX*LVX + 1
          IND = NPVECT + IW(NPVECT+1) + IW(NPVECT+2)*NPARMC
     ...
     *          IW(IND+7).LT.-100 ) THEN
              ...
              RW(NPZE4V + NPMC + 3) = RW(IND+3)
              RW(NPZE4V + NPMC + 4) = RW(IND+4)
              NPMC = NPMC + 4
            ENDIF
4110      CONTINUE
          DO 4111 IMC=NJET+1,NPRONM
            RW(NPZE4V + NPMC + 1) = 0.
            RW(NPZE4V + NPMC + 2) = 0.
            RW(NPZE4V + NPMC + 3) = 0.
            RW(NPZE4V + NPMC + 4) = 0.
...
9212      FORMAT(' NO VECT BANK IN EVENT ',I8,'  RUN ',I8)
        ENDIF
        NPMC = LH + NVX*LVX
        HW((NPZE4V+NPMC)*2+1) = NJET
        HW((NPZE4V+NPMC)*2+2) = NFLAV
C
      ENDIF
C                                           END OF MC PART
C
...
C
C                                           BANK ZE4V
*** PMF 09/12/99      ENTRY ZE4VST( NST, N, IGEN )
*** This ENTRY point is not called by any of the ZE4V procedures.
*** Comment this because of segmentation fault problems concerning the
*** variables N and NST !!! 
C-----------------------------------------------------------
