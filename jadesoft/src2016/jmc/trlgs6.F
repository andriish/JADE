C   20/01/83 705071225  MEMBER NAME  TRLGS6   (S)           FORTRAN
...
C    UPDATE ENDCAP POS.VECTOR WITH 20 % EXTRA, FOR PRECISION IN TRLGL
C                                   4.9.86   J.OLSSON
C
C    CALL TO VEGAS REPLACED BY CALL TO WEGAS (COMMON TO TRLGS6,TRLGSH)
C    FNORM FACTORS UPDATED ACC. TO F11MAG.CLUS VALUES
C    RENAME EXPECM TO EXPEC6, TO ENABLE RUNNING AS TRLGS6 TOGETHER WITH
C    TRLGSH, WHICH CALLS ANOTHER EXPECM...    17.11.1986  J.O.
C      CHANGES      19/01/87    J.OLSSON/N.MAGNUSSEN
C      2. FNORM IN EXPECM CHANGED TO 59000 AND 21220 (INTEGRATION VALUE)
C
C   CHANGE OF CODE IN SF65, TO PREVENT NEG. ARGUMENT IN ALOG
C                                       30.1.87    N.M / J.O
C   CHANGE OF CODE IN SF56, FOR SIMILAR REASONS..
C                                        2.2.87    N.M / J.O
C   CHANGE OF DIMENSION IN LISTBL,BLCFRC
C                                        1.4.87    D.P / J.O
C   AVGSUM NORM IN ENERGY SUM
C                                       13.4.87    D.P / J.O
...
C
      ...
      WRITE(6,1113)
1113  FORMAT(' ===>> TRLGS6: DOUBLE TRACKING CORRECTED VERSION 4.9.86 '/
...
C
C     CALCULATE EXIT POINT AT THE BACK SURFACE OF THE LEAD GLASS
C
      R0(1)=R(1)+PVX*TRKL
      R0(2)=R(2)+PVY*TRKL
      R0(3)=R(3)+PVZ*TRKL
...
C ****    EXPEC6 USES BLOCK COUNTING STARTING AT 0
      DO 532 N=1,NO
  532 LISTBL(N) = LISTBL(N) - 1
      CALL EXPEC6(IPART,ENERGY,VAR,DC,NO,LISTBL,BLCFRC,AVGSUM)
...
C ***  UPDATE BLOCK ENERGIES
      DO 531 NB=1,NO
      NBL = LISTBL(NB) + 1
C     AMPLG(NBL) = AMPLG(NBL) + READCR*ENERGY*BLCFRC(NB)
...
C
  730 IPART = 1
      ...
      VAR(2)= R(2)
      IF(LISTBL(1).LT.0) RETURN 1
C *****     EXPEC6 USES USUAL BLOCK COUNTING STARTING FROM 0
      DO 732 N=1,NO
...
C ***  UPDATE BLOCK ENERGIES
  ...
  731 CONTINUE
C
C UPDATE POSITION VECTOR TO BACK END OF LG, 20 % EXTRA FOR SECURITY
C
...
C
      RETURN
      END
      SUBROUTINE EXPEC6(IPART,E,VAR,DIRC,NBLOCK,IBLIST,BLCFRC,AVGSUM)
C
...
C
      ...
      DATA AD5    /  1.34 ,  1.21 ,  0.26 ,   .66 ,   1.91 /
...
C
      P2A15=PLO25+1.
      P2A16=PLO26+1.
C     FNORM5 = FACMC*60500.0*PLO15*GAMMA(P2A15)/PLO35**P2A15
C     FNORM6 = FACMC*22000.0*PLO16*GAMMA(P2A16)/PLO36**P2A16
C     FNORM5 = FACMC*59800.0*PLO15*GAMMA(P2A15)/PLO35**P2A15
C     FNORM6 = FACMC*21800.0*PLO16*GAMMA(P2A16)/PLO36**P2A16
C     FNORM5 = FACMC*62618.0*PLO15*GAMMA(P2A15)/PLO35**P2A15
C     FNORM6 = FACMC*21936.0*PLO16*GAMMA(P2A16)/PLO36**P2A16
      FNORM5 = FACMC*59000.0*PLO15*GAMMA(P2A15)/PLO35**P2A15
      FNORM6 = FACMC*21220.0*PLO16*GAMMA(P2A16)/PLO36**P2A16
C
C     INTEGRATION
C
C
...
C
  113 MED5=0
      ...
      DO 1131 IB=1,NBLOCK
       ...
       IWHICH = 5
       J=MOD(IBLOCK,32)
       IF(J.GE.13.AND.J.LE.18.AND.IPART.EQ.0) IWHICH = 6
       MED = IWHICH
       IF(MED.EQ.5) MED5=1
       IF(MED.EQ.6) MED6=1
 1131 CONTINUE
...
1146  CONTINUE
      DO 1142 IBL=1,NBLOCK
       ...
       AVG = AVG/FNORM6
       SD = SD/FNORM6
C     IF (ICO.GT.1000) GOTO 2135
C     ICO=ICO+1
C     ESEE=E*AVG
C     WRITE(6,2134) IBLOCK,AVG,SD,C2,ESEE,XMOV6,YMOV6,ZMOV6
C2134 FORMAT(10X,'*--> WEGAS INTEGRAL RESULT SF6#: ',I8,F12.5,/,
C    *       10X,'*--> ERROR + CHI**2 VALUE+ESEEN : ',3F12.5,/,
C    *       10X,'*--> X,Y,ZMOV6: ',3F12.5)
C2135 CONTINUE
       BLCFRC(IBL) = AVG
       AVGSUM = AVGSUM+AVG
1142  CONTINUE
...
1147  CONTINUE
      ...
      DO 1143 IBL=1,NBLOCK
       ...
       IWHICH = 5
       JJ=MOD(IBLOCK,32)
       IF(JJ.GE.13.AND.JJ.LE.18.AND.IPART.EQ.0) IWHICH = 6
       MEDNOW=IWHICH
...
C START IN SF6 AND NOW IN SF6
       ...
       AVG = AVG/FNORM6
       SD = SD/FNORM6
C     IF (ICO.GT.1000) GOTO 1135
C     ICO=ICO+1
C     ESEE=E*AVG
C     WRITE(6,1134) IBLOCK,AVG,SD,C2,ESEE
C1134 FORMAT(10X,'---> WEGAS INTEGRAL RESULT SF6#: ',I8,F12.5,/,
C    *       10X,'---> ERROR + CHI**2 VALUE+ESEEN : ',3F12.5)
C1135 CONTINUE
       BLCFRC(IBL) = AVG
       AVGSUM = AVGSUM+AVG
...
1149   XU(1) = XU(1) + BEHIN5
       ...
       AVG = AVG/FNORM5
       SD = SD/FNORM5
C     IF (ICO.GT.1000) GOTO 1335
C     ICO=ICO+1
C     ESEE=E*AVG
C     WRITE(6,1334) IBLOCK,AVG,SD,C2,ESEE
C1334 FORMAT(10X,'---> WEGAS INTEGRAL RESULT SF5#: ',I8,F12.5,/,
C    *       10X,'---> ERROR + CHI**2 VALUE+ESEEN : ',3F12.5)
C1335 CONTINUE
       BLCFRC(IBL) = AVG
       AVGSUM = AVGSUM+AVG
...
1159  XU(1) = XU(1) + BEHIN5
      ...
      AVG = AVG/FNORM5
      SD = SD/FNORM5
C     IF (ICO.GT.1000) GOTO 1137
C     ICO=ICO+1
C     ESEE=E*AVG
C     WRITE(6,1136) IBLOCK,AVG,SD,C2,ESEE
C1136 FORMAT(10X,'---> WEGAS INTEGRAL RESULT SF65#: ',I8,F12.5,/,
...
C1336 FORMAT(10X,'---> WEGAS INTEGRAL RESULT SF56#: ',I8,F12.5,/,
C    *       10X,'---> ERROR + CHI**2 VALUE+ESEE  : ',3F12.5)
C1137 CONTINUE
      BLCFRC(IBL) = AVG
      AVGSUM = AVGSUM+AVG
...
C
    2 XLTR = XDET-XMOV6
      YLTR = YDET-YMOV6
C     IF(ZMOV6.LT.0.)
C    *WRITE (6,9001) JPART,XDET,YDET,ZDET,XMOV6,YMOV6,ZMOV6
C
C     DETECTOR-COORDINATES OF PUNCHTHROUGH
C     INTEGRATION NOW IN SF5-BLOCK: POSITIVE Z-SIDE
C
      ZLTR = ZDET-ZMOV6
      IF(ZDET.LT.0.) GOTO 200
      DZ1 = ZSF6-ZMOV6
      DZ2 = ZDET-ZSF6
C     ZSHWP = DZ1/CTR
      GOTO 300
C     INTEGRATION NOW IN SF5-BLOCK: NEGATIVE Z-SIDE
 200  DZ1 = ZSF6+ZMOV6
      DZ2 = -ZSF6-ZDET
 300  CONTINUE
      ZSHWP = ABS(DZ1/CTR)
C     IF(ZMOV6.LT.0.)
C    *WRITE (6,9002) XLTR,YLTR,ZLTR,ZSHWP,DZ1,DZ2
C     IF(ZMOV6.LT.0.)
C    *WRITE (6,9004) STR,CTR,SPR,CPR
C
C     ROTATE INTO SHOWER SYSTEM (FIRST CHECK SIGN OF ZSHW)
C
      ZSHW  = XLTR * STRCPR   +  YLTR * STRSPR   +  ZLTR * CTR
...
C     SCALE IN RADIATION LENGTHS
C
      IF(ZSHWP.LT.0.) ICAL65 = ICAL65 + 1
      IF(ZSHWP.LT.0..AND.ICAL65.LT.100)
     *WRITE (6,9002) XLTR,YLTR,ZLTR,ZSHWP,DZ1,DZ2
      IF(ZSHWP.LT.0..AND.ICAL65.LT.100)
     *WRITE (6,9004) STR,CTR,SPR,CPR
      ZSHWP = ABS(ZSHWP)/X0(2)
      ZSHS = ZSHW/X0(2)
      DELZ = ZSHW - ZSHWP
      IF(DELZ.LE.0.) GOTO 20
      ZSHS = ZSHWP/X0(2) + DELZ/X0(1)
C  DEVIDE RSHW ACCORDING TO DZ1,DZ2
 20   RSHW = SQRT(XSHW*XSHW+YSHW*YSHW)
      RSHW1 = 0.
      IF(ZSHW.GT.ZSHWP) GOTO 25
      RSHW0 = DZ1/STR
      RSHW1 = -ZSHW*CTR/STR
      IF(CTR.LT.0.) RSHW1 = ZSHW*CTR/STR
      RSHW1 = RSHW1 + RSHW0
25    RSHW2 = RSHW - RSHW1
      RSHS = RSHW1/X0(2)+RSHW2/X0(1)
C
C     IF(DELZ.GT.0.) RSHS = RSHS*X0(2)/X0(1)
C     IF(ZMOV6.LT.0.)
      IF(RSHW1.LT.0..OR.RSHW2.LT.0.)
     *WRITE (6,9004) STR,CTR,SPR,CPR
      IF(RSHW1.LT.0..OR.RSHW2.LT.0.)
     *WRITE (6,9002) XLTR,YLTR,ZLTR,ZSHWP,DZ1,DZ2
      IF(RSHW1.LT.0..OR.RSHW2.LT.0.)
     *WRITE(6,9006) XSHW,YSHW,RSHS,ZSHS,RSHW1,RSHW2,RSHW
      IF(MEDNOW.EQ.5) GOTO 999
      IF(IC.GE.100) GOTO 999
C      IF(MEDNOW.NE.5) WRITE (6,6000) MEDNOW
      IC=IC+1
  999 CONTINUE
C
C     COMPRESS SHOWER FUNCTION
C
      ...
      EX2 = -1.*(REX+P3Z+P(6,1)*RSHS)
C     IF(ZMOV6.LT.0.)
C    *WRITE(6,9007) EX1,EX2,P3Z,REX,P(3,1),P(1,1)
      IF(EX1.LT.-10..OR.EX2.LT.-10.) RETURN
C
...
C
 ...
 9001 FORMAT(' JPART,XDET,YDET,ZDET,XMOV6,YMOV6,ZMOV6  ',I6,6F10.4)
 9002 FORMAT(' XLTR,YLTR,ZLTR,ZSHWP,DZ1,DZ2           ',6F12.4)
 9004 FORMAT(' STR,CTR,SPR,CPR      ',4F10.5)
 9005 FORMAT(' ZSHW   ',1F12.5)
 9006 FORMAT(' XSHW,YSHW,RSHS,ZSHS,RSW1,RSHW2,RSHW '/7F10.5)
 9007 FORMAT(' EX1,EX2,P3Z,REX,P(3,2),P(1,2) ',6F12.7)
 9008 FORMAT(' S F 6 5 ',1E12.6)
C
      RETURN
      END
C
...
C                     CHANGED   N.M.    26/08/85
C        CHANGED TO MC VERSION  N.M.    11/10/85
C        CHANGED AGAIN 2.2.87          N.M. / J.O.
C
      COMMON /TRAFO/ STR,CTR,SPR,CPR,CTRCPR,CTRSPR,STRCPR,STRSPR,
     *               JPART,XMOV5,YMOV5,ZMOV5,XMOV6,YMOV6,ZMOV6
      COMMON /PARAME/ P(6,2),MEDNOW
...
C
    2 XLTR = XDET-XMOV5
      YLTR = YDET-YMOV5
C     WRITE (6,9001) JPART,XDET,YDET,ZDET,XMOV5,YMOV5,ZMOV5
C
C     DETECTOR-COORDINATES OF PUNCHTHROUGH
C     INTEGRATION NOW IN SF5-BLOCK: POSITIVE Z-SIDE
C
...
C  DEVIDE RSHW ACCORDING TO DZ1,DZ2
      ...
      RSHW1 = RSHW*ABS(DZ1)/(ABS(DZ1)+ABS(DZ2))
      RSHW2 = RSHW - RSHW1
      RSHS = RSHW1/X0(1)+RSHW2/X0(2)
C
      IF(RSHW1.LT.0..OR.RSHW2.LT.0.) ICAL56 = ICAL56 + 1
      IF(ICAL56.GT.100) GO TO 1304
C
      IF(RSHW1.LT.0..OR.RSHW2.LT.0.)
     *WRITE (6,9001) JPART,XDET,YDET,ZDET,XMOV5,YMOV5,ZMOV5
      IF(RSHW1.LT.0..OR.RSHW2.LT.0.)
     *WRITE (6,9004) STR,CTR,SPR,CPR
      IF(RSHW1.LT.0..OR.RSHW2.LT.0.)
     *WRITE (6,9002) XLTR,YLTR,ZLTR,ZSHWP,DZ1,DZ2
...
C
      ...
      EX1 = -1.*(REX+P3Z+P(4,2)*RSHS)
      EX2 = -1.*(REX+P3Z+P(6,2)*RSHS)
C     WRITE(6,9007) EX1,EX2,P3Z,REX,P(3,2),P(1,2)
      IF(EX1.LT.-10..OR.EX2.LT.-10.) RETURN
C
...
C
 4    SF56 = SF56*FJACOB
 9001 FORMAT(' JPART,XDET,YDET,ZDET,XMOV6,YMOV6,ZMOV6  ',I6,6F10.4)
 9002 FORMAT(' XLTR,YLTR,ZLTR            ',3F12.4)
 9004 FORMAT(' STR,CTR,SPR,CPR      ',4F10.5)
 9005 FORMAT(' ZSHW   ',1F12.5)
 9006 FORMAT(' XSHW,YSHW,RSHS,ZSHS,RSHW1,RSHW2,RSHW '/7F10.5)
 9007 FORMAT(' EX1,EX2,P3Z,REX,P(3,2),P(1,2) ',6F12.7)
 9008 FORMAT(' S F 5 6 ',1E12.6)
