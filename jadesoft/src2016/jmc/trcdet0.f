C   09/11/84            MEMBER NAME  TRCDET0  (S)           FORTRAN
C
C-----------------------------------------------------------------------
...
C
      ...
      PENETZ = 0.
      TOTLEN = 0.
      STPLEN = 1.E33 ! PMF 06/11/98 1.E40
C
...
C                            BEAM PIPE COUNTER
  400 CALL JPRBPC( R, P, PENETR, PENETZ, RBPC , RBPC + DRBPC,
     *             ZBPMI, ZBPPL, POTTRI, ZAROTR, DRBPC/XRLBPC,
     *                   *300, *8500, *7500, *9100 )
C
...
CCC  TRACK PARTICLES THRU JETCHAMBER
      CALL JCHAMB( R, P, PENETR, PENETZ, IRETRN, *9400 )
C
CCC  UPDATE CJPATR COMMON
      CALL JUPATR( P, R )
C
CCC  BRANCH ACCORDIING TO RETURN CODE FROM JCHAMB
      GO TO ( 700, 8000, 7000, 9100 ) , IRETRN
C
C
