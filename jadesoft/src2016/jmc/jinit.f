C   03/12/83 606101227  MEMBER NAME  JINIT9   (S)           FORTRAN
C
C-----------------------------------------------------------------------
      SUBROUTINE  JINIT
C-----------------------------------------------------------------------
...
C        MOD  C. BOWDERY   2/12/83 :  CHANGE /CVFLAG/
C   LAST MOD  J. HAGEMANN 21/09/84 :  INITIALIZE VERTEX CHAMBER DRIFT
C                                     SPACE CONSTANTS
C
C-----------------------------------------------------------------------
C
...
C                                 VERTEX CHAMBER
      IF ( .NOT. LFLAG(5) ) GO TO 5
         BET    = 3.1415927 / FLOAT( MCELL )
         SINBET = SIN( BET )
         COSBET = COS( BET )
C
         YSLOPV = SINBET / COSBET
         XSLOPV = - YSLOPV
C
         XLV    = RPIPV + DRPIPV
         XHV    = RVXC
