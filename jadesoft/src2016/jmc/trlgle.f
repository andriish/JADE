C   20/03/81 609041935  MEMBER NAME  TRLGLE   (S)           FORTRAN
...
C ==> CHANGED :  USE LFLAG TO STEER SHOWER CALCULATION.
C J.O./ 6.6.1986:  MODIFIED TO INCLUDE MAGNUSSENS SF5/SF6 SHOWER SIM.
C J.O./ 4.9.1986:  ERROR PRINT TO TRACE DOUBLE TRACKING IN ENDCAP
C
...
C                                           OF THE END CAP COUNTERS
 1000 CONTINUE
      ...
      RABS=SQRT(RABS)
      IF(LFLAG(4)) WRITE(6,2101) R(1),R(2),R(3),RABS,PV(4)
2101  FORMAT(' DOUBLE TRACKING!  R',3E12.4,' RABS ',E12.4,' PV4 ',E12.4)
      R(1)=R(1)/RABS
      R(2)=R(2)/RABS
      R(3)=R(3)/RABS
      DISTZ=ZENDPL
