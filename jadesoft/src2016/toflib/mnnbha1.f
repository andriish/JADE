C   08/04/82         5  MEMBER NAME  MNNBHA1  (S)           FORTRAN
      SUBROUTINE MNNBHA
      INTEGER *2 HDATA
...
C FLAT CABLES INTER CHANGED 25-21  29-35
      ...
      NAC = NAC + 1
      CALL ELIM(NRUN,IEV,1,LOUT)
      IF(LOUT.NE.0) GOTO  17
  ...
  704 FORMAT('  RUNS  ',20I5)
   ...
   73 CALL KLREAD(KALRET)
      IF(KALRET.NE.0) GOTO 300
      CALL SUBCAL
   74 IRUN = NRUN
      ITYP=HDATA(2*IPHEAD+12)
      IF(ITYP.GE. 64) GOTO  2
      CALL TRPLOT(IANDOR,IPTRAC)
      IPLTCH = IDATA(IQLTCH)
      IF(IPLTCH.LE.0)  GOTO  2002
      IPPATR = IDATA(IQPATR)
      IF(IPPATR.LE.0) GOTO  2003
      CALL STOTRK(IPPATR,0,1,0,1)
      IF(NFIN.NE.1)  GOTO  2
C
      CALL LGANAL
      IPCLST = IDATA(IQCLST)
      IF(IPCLST.LE.0)  GOTO  2004
      ETOTB = RDATA(IPCLST+12)
      CALL HF1(30,ETOTB,1.)

C     IF(ETOTB.LT.10.)  GOTO  2
      IPLGN = IDATA(IQALGN)
      IF(IPLGN.LE.0)  GOTO  2005
      CALL LGCDIR(IPPATR,IPLGN,IPCLST)
      IPTOF = IDATA(IQTOFR)
      IF(IPTOF .LE.0)  GOTO  401
      CALL TOFINT(NRUN,IPTOF,IPPATR,&3000)
      CALL HF1(600,50.,1.)
C     IPTOFR = IDATA(IBLN('TOFR'))
C     IF(IPTOFR.LE.0)  GOTO  402
C     CALL COMPR(NRUN)
C     CALL LATCH
C     CALL TFUNPK(IPTOFR,1)
   22 IWRITE = IWRITE+ 1
      IF(IWRITE.LE.20) PRINT 503,IWRITE,NRUN,IEV,NEV,NMU
  503 FORMAT(' WRITTEN',10I10)
C     IF(IWRITE.LE.20) CALL PRTOFR(IPTOFR)
