C   13/12/81 306071433  MEMBER NAME  MNNPRO   (S)           FORTRAN
      SUBROUTINE MNNPRO
      INTEGER *2 HDATA
...
C
      ...
      CALL STBUK(40000)
      CALL HBOOK1(55,' STATISTICS MAIN$',100,0.,100.)
      NAC = 0
      NEV = 0
      NEVA= 0
      NEVB= 80000
...
C
  ...
  77  CONTINUE
   ...
   55 CONTINUE
      ...
      NEV = NEV + 1
      IF(NRUN.LT.13542)  GOTO  2
      IF(NEV.LT.NEVA)  GOTO  2
...
C     IF(LOUT.NE.0) GOTO  17
  ...
  704 FORMAT('  RUNS  ',20I5)
   ...
   74 IRUN = NRUN
      ...
      CALL HF1(55,5.,1.)
      CALL STOTRK(IPPATR,0,1,0,1)
      IF(NFIN.LT.1)  GOTO  2
      CALL HF1(55,10.,1.)
C
      CALL BLOC(IPTOF,'ATOF',0,&2005)
      ...
      CALL LGANAL
      IPCLST = IDATA(IBLN('LGCL'))
      IF(IPCLST.LE.0)  GOTO   402
      CALL BLOC(IPLGN,'ALGN',1,&403)
      CALL LGCDIR(IPPATR,IPLGN,IPCLST)
      CALL LATCH
      CALL TFUNPK(IPTOFR,1)
      IF(.NOT.EVWRIT) GOTO  2
C    RECALIBRATE Z
C     CALL ZSSPEC(NRUN,LUNZ)
C     IJETC0 = IDATA(IQJETC)
C     IF(IJETC0.LE.0)  GOTO  2003
C     IJETC1 = IDATA(IJETC0-1)
C     IF(IJETC1.LE.0)  GOTO  2004
C     IPJHTL = IDATA(IQJHTL)
C     IF(IPJHTL.LE.0)  GOTO  403
C     CALL ZSFIT(IJETC0,IJETC1,IPJHTL,IPPATR,0)
   22 IWRITE = IWRITE+ 1
