C   18/05/82 205211405  MEMBER NAME  TFAR5    (S)           FORTRAN
...
C
      DATA IENTRY/ 0/,IBUG/ 8/
      DATA ADCLIM/1./
C
      IF(ITRC(1).NE.2.AND.ITRC(2).NE.2)  RETURN
      NRUN = HEADR(18)
      IF(IENTRY.NE.0)  GOTO 320
      IENTRY = IENTRY + 1
      ZA = 10050
      ZB = 10250
      CALL HBOOK2(445,'  NRUN MULT1        $',100,ZA,ZB,42,0.,42.)
      PRINT 101,NRUN
      PRINT 102,TCALM2
      PRINT 102,TCALP2
      PRINT 102,TCPDM
...
C//////////
      ...
      MULT1 = 0
      DO   45   N=1,42
      IF(HON(N).NE.0) MULT1 = MULT1 + 1
   45 CONTINUE
      CALL HFILL(445,FLOAT(NRUN),FLOAT(MULT1))
C
      DO 3000 NCN= 1,42
      IBUG = 11
      IF(NCN.EQ.2) IBUG = 1
...
C
   ...
   20 CONTINUE
      CMS = 40.+TCPDM(NCN)-3.5
      CPS = 40.+TCPDP(NCN)-3.5
C     IF(ADCM.LT.1024.) CMS = CORNOR(2*NCN-1)
C     IF(ADCP.LT.1024.) CPS = CORNOR(2*NCN)
C     IF(ADCM.GE.1024.) CMS = COROVF(2*NCN-1)
C     IF(ADCP.GE.1024.) CPS = COROVF(2*NCN)
C
      IF(IBUG.LE.10) PRINT 106,NCN,ADCM,ADCP,TDCM,TDCP,CMS,CPS
  ...
  106 FORMAT(' TOFARD',I5,10F10.3)
     ...
     *TDCM20= TDCM2*TCALM2(NCN)-TCPDM2(NCN)-42.5
      IF(IBUG.LE.10) PRINT 116,HON
  116 FORMAT(' HON TFAR5',42I2)
