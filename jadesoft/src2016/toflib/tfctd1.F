C   04/03/80 704231727  MEMBER NAME  TFCTD1   (S)           FORTRAN
...
C
       DATA PAI/ 3.1415927/ ,IBUG/100/,JSQRT/10/
...
C
      ...
      R = 1./DATA(IP+25)
      ITYPE = IDATA(IP+18)
      IF(IBUG.LE.10) PRINT 111,ITYPE,R
  101 FORMAT(1X,2I5,10F12.5)
  111 FORMAT(1X,I5,10F12.5)
...
C
      ...
      IF(IBUG.LE.0) PRINT 101,IDATA(IP+18),RC,D0,ALPHC,XM,YM,XCL,YCL
      GOTO  31
C
C PARABOLA
   30 CONTINUE
   ...
   33 XCL = DS*COS(ALPH)
      YCL = DS*SIN(ALPH)
      IF(IBUG.LE.0) PRINT 101,IDATA(IP+18),RP,ALPHP,X0,Y0,XM,YM,XCL,
     1 YCL
   31 CONTINUE
...
C
  ...
  650 XTE = 0.
      YTE = RTOF
  651 IF(IBUG.LE.0) PRINT 102 ,XTE,YTE
...
C TAKE SOLUTION WHICH IS CLOSER TO STRAIGHT EXTRAPOLATION
    ...
    9 IF(IBUG.LE.0) PRINT 102,YT1,XT11,XT12,DC11,DC12,YT2,XT21,XT22,
     1 DC21,DC22
...
C
      ...
      IF(IBUG.LE.0) PRINT 102,DE1,DE2
      YT = YT1
  ...
  103 FORMAT(' NEG SQRT ',5I10)
      ...
      GOTO  1000
  988 JSQRT = JSQRT + 1
      IF(JSQRT.LE.0) PRINT 507,RTOF,YT
  507 FORMAT(' RTOF YT  --  NEG SQRT ',2E12.3)
      IF(JSQRT.LE.0) PRINT 102,A,P,Q
      IF(JSQRT.LE.0) PRINT 102,YT1,YT2,DY1,DY2,YT
      GOTO  1000
C
C=========   PATH LENGTH
C
   ...
   52 SEHNE = SQRT((XT-XCL)**2+(YT-YCL)**2)
      ...
      IF(IBUG.LE.0) PRINT 102,SEHNE,PATHR,PATH,PHAI
C/////////
C
...
C---   ITRC(50) : TRACK # I <=> COUNTER # NCNT
 ...
 310        ITRC(I)= NCNT
      IF(IBUG.LE.10) PRINT 101,I,NCNT,ZCL
      IF(IBUG.LE.10) PRINT 111,I,XTOF(I),YTOF(I),ZTOF(I)
