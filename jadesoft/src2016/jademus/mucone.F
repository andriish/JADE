C   22/04/85 504230956  MEMBER NAME  MUCONE   (JADEMUS)     FORTRAN
C   22/04/85 504221657  MEMBER NAME  MUCONE2  (MUCALIB.S)   FORTRAN
C   12/07/84 408101051  MEMBER NAME  MUCONE1  (PWMUS)       FORTRAN
C   06/10/82 408101051  MEMBER NAME  MUCONE1  (MUCALIB.S)   FORTRAN
...
C
C LAST CHANGE 14.30 22/04/85 P. WARMING   - CTLO/CTHI HANDLING CORRECTED
C                                 VARIOUS BUGS CORRECTED FOR UPDATE-MODE
C                                         --> NEEDS IMPROVED 'MUCHAN'
...
C
     ...
     * 'BLLO','BLHI','BTLO','BTHI','BNLM','ACTW','AANG'/
      DATA NQ/21/
C
C-----------------------------------------------------------------------
C
...
C READ NEW VERSION DESCRIPTION.(TERMINATE WITH ^.)
 ...
 106  FORMAT('0OLD VERSION',I6,5X,15A4)
      IF(NVERSN.EQ.0) WRITE(6,107)
 107  FORMAT('+',T25,'  =========  UPDATE MODE  =========')
...
C
C-----------------------------------------------------------------------
      IF (IFLAG.EQ.0) NFRAMS = 82
      IF (IFLAG.EQ.0) NCHAMS = 634
C-----------------------------------------------------------------------
C
C PRINT HEADING.
...
C I1.EQ.0 - SET IFRCH2 ACCORDINGLY.
      GO TO (7,8,8,8,8,8,8,9,9,9,9,9,9,9,10,10,10,10,10,9,8),I
 7    CONTINUE
      IFRCH2=5
      GO TO 12
 8    CONTINUE
...
C EXECUTE EDITS.
 14   CONTINUE
     ...
     * 1011,1012,1013,1014,1015,1016,1017,1018,1019,1020,1021),I
 1000 CONTINUE
...
C-----------------------------------------------------------------------
C
C ADD CONSTANT TO FRAME TWISTS.
 1021 CONTINUE
      IF (IFLAG.EQ.0) GOTO 97
      DO 2021 I=IFRCH1,IFRCH2
      HANG(I)=HANG(I)+INEW
 2021 CONTINUE
      GO TO 4
C
...
C
 97   CONTINUE
      CALL MUERRY('MUCONE',0,'ILLEGAL EDIT COMMAND.^')
      GO TO 4
C
C-----------------------------------------------------------------------
C ERROR CONDITIONS,ETC.
...
C     FIX UP CTLO AND CTHI.
  ...
  192 CONTINUE
      GOTO 199
  195 IF (IFLAG.NE.0) GOTO 199
      WRITE(6,119)
  119 FORMAT(/' ////////////// UPDATE MODE: CTLO/CTHI HANDLED BY',
     *        ' MUCHAN (NEW VERSION) /////////////'/)
        J=0
        IALPH=6
        CALL MUCHAN(J)
CCCC    IALPH=7                DONE IN PARALLEL WITH IALPH=6
CCCC    CALL MUCHAN(J)
  199 CONTINUE
      REWIND LUNMUE
      RETURN
