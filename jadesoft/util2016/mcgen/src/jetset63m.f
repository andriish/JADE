...
*     Handle with rounding errors in PAWT leading
*     to a negativ SQRT-argument in PAWT
      IF( PV(IL,5) .LT. PV(IL+1,5)+P(N+IL,5)  ) THEN
         PRINT *,'LUDECY: PV(IL,5)=',PV(IL,5)
     +          ,' PV(IL+1,5)+P(N+IL,5)=',PV(IL+1,5)+P(N+IL,5)
*---  set very small negative arguments of SQRT in PAWT to zero
         IF( PV(IL,5)**2-(PV(IL+1,5)+P(N+IL,5))**2 .GE. -.1E-06 ) THEN
            PRINT *,'... will set PAWT to 0.'
            WT=0.
         ELSE
*---  stop if argument of SQRT in PAWT is more negative
            PRINT *,'... serious problems in PAWT! Will stop now!'
            STOP
         ENDIF
      ELSE
*--- normal case
