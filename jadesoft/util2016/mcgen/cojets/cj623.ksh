#!/usr/bin/ksh
# Job to generate 4-vector MC with COJETS / PYTHIA 5.7
# Modifications:
...
+PAM,13,R=COJCDE,T=A,C. coj623d.car
+PAM,14,T=A,C. mc$VERSION.car
+PAM,15,T=A,C. pymaxi.car
+QUIT.
EOF

# Run patchy to get the code:
ytofort mccj623_$VERSION
rc=$?
if [ $rc -ne 0 ] ; then
     echo 'cj623_qq: patchy return code' $rc
     echo `whoami` -m 'cj623_qq: patchy wrong'
     exit $rc
fi
# Compile, output goes in subdirectory:
fort mccj623_$VERSION.f
rc=$?
if [ $rc -ne 0 ] ; then
     echo 'cj623_qq: compilation return code' $rc
     echo `whoami` -m 'cj623_qq: compilation wrong'
     exit $rc
fi
...
if [ $HARDWARE = "RS6000" ]; then
    ...
    ulink="-uludata_ -umcevbl_ -umcpobl_"
fi
link -m -D $ulink mccj623_$VERSION.o \
	libmc$VERSION libcoj623d \
	libpythia5722 libjt74opal \
	libckern105 libpx114
rc=$?
if [ $rc -ne 0 ] ; then
  echo 'cj623_qq: bad news, link return code' $rc
  echo `whoami` -m 'mc-comp_link: no executable made'
  exit $rc
fi
mkdir -p ${JADE_MCBIN}
if [ -x mccj623_$VERSION.run ]; then
    mv -f mccj623_$VERSION.run ${JADE_MCBIN}/.
    echo "mccj623_$VERSION.run moved to ${JADE_MCBIN}/."
else
...
rm -f mccj623_$VERSION.cra
rm -f mccj623_$VERSION.lis
#rm -f mccj623_$VERSION.f
rm -f mccj623_$VERSION.o
exit
...
# For COJETS job, set here some MC parameters
ECM=35.0
MCNEVT=100

# Execute:
time ${JADE_MCBIN}/mccj623_$VERSION.run << EOF
E+E- ANNIHILATION INTO U,D,S,C,B QUARKS 
${ECM},${MCNEVT},1,1  
E+E-    
KEEQED
0
