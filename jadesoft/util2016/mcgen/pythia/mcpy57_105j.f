...
*. 27.07.00, PMF: Add initialisation of Peterson FF 
**.
*.********************************************************************
...
C  Standard OPAL jetset parameters:
      ...
      WRITE(CH8,FMT='(F8.5)') JTEC
      CALL LUGIVE('PARJ(54)='//CH8)
      WRITE(CH8,FMT='(F8.5)') JTEB
      CALL LUGIVE('PARJ(55)='//CH8)

      WRITE(CH8,FMT='(F8.5)') JTA
      CALL LUGIVE('PARJ(41)='//CH8)
      WRITE(CH8,FMT='(F8.5)') JTLAMB
      CALL LUGIVE('PARJ(81)='//CH8)
...
C PMF12.06.01:  MSTP(71) is a switch for QED and QCD FSR!!
C               LFSR=OFF would also prohibit QCD parton showers!
C     IF( LFSR ) THEN
C       CALL LUGIVE('MSTP(71)=1')
C     ELSE
C       CALL LUGIVE('MSTP(71)=0')
C     ENDIF
...
C
C  Overwrite with special JADE tune (also disables L=1 multipletts etc)
      IF( JADETUN.NE.0 ) CALL JATUNE
C
