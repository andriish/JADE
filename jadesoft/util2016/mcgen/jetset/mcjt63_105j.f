...
*.********************************************************************
*.
*. Stand alone driver routine for Jetset 6.3.
*.
*. 28.05.01, P.A. Movilla Fernandez
*.
*.********************************************************************
      INTEGER MST63
      REAL PAR63
      ...
      INTEGER MSTE63
      REAL PARE63
      COMMON/LUDATE/MSTE63(40),PARE63(80)
      SAVE /LUDATE/
...
C  Stuff for FFREAD:
     ...
     &     ,CHMCTYP,CHEVFILE
      SAVE /MCFFR/
      INTEGER IEVT,IFLAV,IERR,IREP
C
      EXTERNAL LUDA63,LUHDAT,LUEDAT
C
...
C  User initialisation:
      ...
      CHMCTYP= 'LUND63'
C
C  Update JETSET parameters, switches and decay table from JADE default:
      PRINT*,' '
      PRINT*,'JADE JETSET 6.3 default:'
      PRINT*,' '
      CALL JT63JA
      PRINT*,' '
      PRINT*,'Changes from JADE JETSET 6.3 default:'
      PRINT*,' '
      IF(ABS(PAR63(44)-JTEC).GT.1E-6)WRITE(*,'(A22,E12.4,A4,E12.4)') 
     >     'PAR(44) changed from',PAR63(44),' TO',JTEC
      IF(ABS(PAR63(45)-JTEB).GT.1E-6)WRITE(*,'(A22,E12.4,A4,E12.4)') 
     >     'PAR(45) changed from',PAR63(45),' TO',JTEB
      IF(ABS(PAR63(12)-JTSQ).GT.1E-6)WRITE(*,'(A22,E12.4,A4,E12.4)') 
     >     'PAR(12) changed from',PAR63(12),' TO',JTSQ
      IF(ABS(PARE63(22)-JTQ0).GT.1E-6)WRITE(*,'(A22,E12.4,A4,E12.4)') 
     >     'PARE(22) changed from',PARE63(22),' TO',JTQ0
      IF(ABS(PAR63(31)-JTA).GT.1E-6)WRITE(*,'(A22,E12.4,A4,E12.4)') 
     >     'PAR(31) changed from',PAR63(31),' TO',JTA
      IF(ABS(PAR63(32)-JTB).GT.1E-6)WRITE(*,'(A22,E12.4,A4,E12.4)') 
     >     'PAR(32) changed from',PAR63(32),' TO',JTB
      IF(ABS(PARE63(21)-JTLAMB).GT.1E-6)WRITE(*,'(A22,E12.4,A4,E12.4)') 
     >     'PARE(21) changed from',PARE63(21),' TO',JTLAMB
C
C  Set parameter for peterson fragmentation function
      PAR63(44)= JTEC
      PAR63(45)= JTEB
C
C  Set width of transversal momentum distribution
      PAR63(12)= JTSQ
C
C  Set shower mass cutoff
      PARE63(22)= JTQ0
C
C  Set parameter for symmetric lund fragmentation function
      PAR63(31)= JTA
      PAR63(32)= JTB
C
C  Set leading log lambda:
      PARE63(21)= JTLAMB
      PRINT*,' '
C
C  Initial state photon radiation, 0 no isr; 1 for isr,
      PRINT*,'Switches for ISR:'
      IF( LISR ) THEN
         MSTE63(7)= 1
      ELSE
         MSTE63(7)= 0
      ENDIF
      WRITE(*,'(A17,I4)') 'MSTE(7) set to ',MSTE63(7)
C
      WRITE(*,'(/A,L2,A,A)') ' No FSR in Jetset6.3!'
      LFSR=.FALSE.
C
C  Set long-lived hadrons stable if desired:
      PRINT*,' '
      IF( LSTBL ) CALL MCST63
C
...
C       ( infinite loop + momentum conservation(6) )
        IF( MST63(25).EQ.4 .OR. MST63(25).EQ.6 ) THEN
          PRINT*,'MCJT63: JETSET error: ',MST63(25),' regenerate'
          GOTO 10
        ENDIF
C
C       Convert to Jetset74 common blocks
        IREP=0
        IF( IEVT.LE.NRTOPR .OR. MOD(IEVT,5000).EQ.0 ) IREP=1
        CALL JTCONV(IREP,IERR)
        IF( IERR.NE.0 ) THEN
           PRINT*,'MCJT63: conversion JETSET6.3 -> JETSET7.4 failed'
           PRINT*,'        IERR=',IERR
           STOP
        ENDIF
C
...
C ... list the event and make a copy
        CALL LULIST(1)
        CALL LUEDIT(-1)
        IF( MST63(30).EQ.0 ) THEN
           PRINT*,'MCUSEV: Not enough space in /LUJETS/ (Jetset6.3)'
           STOP
        ENDIF
      ENDIF
C
...
C  Mark photons with no parents for removal if demanded, they are ISR:
      IF( LISRRM ) THEN
        IF( ICALL.LE.NRTOPR  .OR. MOD(ICALL,5000).EQ.0 ) THEN
C ... throw away all partons and jets/particles that have fragmented
          CALL LUEDIT(1) 
          PRINT*,' '
          PRINT*,'MCUSEV: Event record before ISR removal:'
C ... list modified event record restore the original one
          CALL LULIST(1)
          CALL LUEDIT(-2)
        ENDIF
        DO I=1,N
