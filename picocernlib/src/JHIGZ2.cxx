...
std::map<int,std::vector<double> > gGlobal;
std::map<int,std::vector<double> > gLocal;
std::map<int,std::vector<double> > gCoef;


std::vector<double>  GLOBAL;

void gTrans(int i, float xlocal, float ylocal, float &xglobal, float & yglobal)
{
//xglobal=(gGlobal[i][1]-gGlobal[i][0])/(gLocal[i][1]-gLocal[i][0])*(xlocal-gLocal[i][0])+gGlobal[i][0];
//yglobal=(gGlobal[i][3]-gGlobal[i][2])/(gLocal[i][3]-gLocal[i][2])*(ylocal-gLocal[i][2])+gGlobal[i][2];

xglobal=gCoef[i][0]+gCoef[i][1]*xlocal;
yglobal=gCoef[i][2]+gCoef[i][3]*ylocal;



}

void gTransN(int id,int N, float* xlocal, float* ylocal, float* xglobal, float * yglobal)
{
for (int i=0;i<N;i++) gTrans(id,xlocal[i],ylocal[i], xglobal[i],  yglobal[i]);
}
...
xglobal=(gGlobal[1]-gGlobal[0])/(gLocal[1]-gLocal[0])*(xlocal-gLocal[0])+gGlobal[0];
yglobal=(gGlobal[3]-gGlobal[2])/(gLocal[3]-gLocal[2])*(ylocal-gLocal[2])+gGlobal[2];
   


*/
//if (NTS==8&&gLocal.find(9)==gLocal.end())gLocal[9]=a;
...
/*
    ...
    TPad* pad= new TPad(pname,"jPad", XTMIN, YTMIN, XTMAX, YTMAX);
    ...
    pad->Draw();
    pad->cd();
    if (gLocal.find(NTS)!=gLocal.end())
    gPad->Range(BIGK*gLocal[NTS][0],BIGK*gLocal[NTS][1],BIGK*gLocal[NTS][2],BIGK*gLocal[NTS][3]);
    else
    printf("Error in void isvp_\n",NTS);
    gPad->Update();
    }
...
int i=NTS;
gGlobal[i]=gGlobal[0];
if (gLocal.find(i)==gLocal.end()) gLocal[i]=gGlobal[i];
std::vector<double> tempg;

tempg.push_back(gGlobal[i][0]+(gGlobal[i][2]-gGlobal[i][0])*XTMIN);//Global XMIN
tempg.push_back(gGlobal[i][1]+(gGlobal[i][3]-gGlobal[i][1])*YTMIN);//Y

tempg.push_back(gGlobal[i][0]+(gGlobal[i][2]-gGlobal[i][0])*XTMAX);//Global XMAX
tempg.push_back(gGlobal[i][1]+(gGlobal[i][3]-gGlobal[i][1])*YTMAX);
...
n.push_back(0);
gCoef[i]=n;
gCoef[i][0]=(tempg[2]-tempg[0])/(gLocal[i][2]-gLocal[i][0])*(-gLocal[i][0])+tempg[0];
gCoef[i][1]=(tempg[2]-tempg[0])/(gLocal[i][2]-gLocal[i][0]);
gCoef[i][2]=(tempg[3]-tempg[1])/(gLocal[i][3]-gLocal[i][1])*(-gLocal[i][1])+tempg[1];
gCoef[i][3]=(tempg[3]-tempg[1])/(gLocal[i][3]-gLocal[i][1]);


printf("gGlobal[i] =%f %f %f %f\n",gGlobal[i][0],gGlobal[i][1],gGlobal[i][2],gGlobal[i][3]);
printf("gLocal[i]  =%f %f %f %f\n",gLocal[i][0],gLocal[i][1],gLocal[i][2],gLocal[i][3]);
printf("gCoef[i]   =%f %f %f %f\n",gCoef[i][0],gCoef[i][1],gCoef[i][2],gCoef[i][3]);
...
//DDDD printf("void iselnt_(int & t)  %i\n",t);
	jNT=t;
//if (gLocal.find(t)!=gLocal.end())
//gPad->Range(BIGK*gLocal[t][0],BIGK*gLocal[t][1],BIGK*gLocal[t][2],BIGK*gLocal[t][3]);
//gPad->Range(gLocal[t][0],gLocal[t][1],gLocal[t][2],gLocal[t][3]);
//else
//{
...
if (jCanvas->GetListOfPrimitives()->FindObject("Jpad")) 
   ((TPad*)(jCanvas->GetListOfPrimitives()->FindObject("Jpad")))->Clear();
...
void igbox_(float& ix1, float & ix2,float& iy1, float & iy2)
{
//DDDD 
printf("void igbox_(float& x1, float & x2,float& y1, float & y2) %f %f %f %f\n",ix1,iy1,ix2,iy2);

float x1,y1,x2,y2;
gTrans(jNT,ix1,iy1,x1,y1);
gTrans(jNT,ix2,iy2,x2,y2);
...
void ixbox_(float& ix1, float & ix2,float& iy1, float & iy2, int mode)
{
//DDDD 
...
//if (!jCanvas)   {
    ...
    jPad->cd();
//    if (gLocal.find(NTS)!=gLocal.end())
jPad->Range(-BIGK*4096,-BIGK*4096,BIGK*4096,BIGK*4096);

//jPad->Range(BIGK*gLocal[jNT][0],BIGK*gLocal[jNT][1],BIGK*gLocal[jNT][2],BIGK*gLocal[jNT][3]);
...
void igpave_(float& ix1, float&  ix2,float& iy1,float& iy2,int*, int*,const char* )
{
...
//`ipl_'
void ipl_(int&n, float* ix, float* iy)
{

float x[100],y[100];
gTransN(jNT,n,ix,iy,x,y);
...
//`ipm_'
void ipm_(int&n, float* ix, float* iy)
{
float x[100],y[100];
gTransN(jNT,n,ix,iy,x,y);
...
void itx_(float &ix, float &iy, char* txt)
{
...
void itxl_(float &ix, float &iy, char* txt, int& l)
{
 //DDDD   printf("void itx_(float &x, float &y, char* txt)   %f %f %s, jTextAlignment=%i  jTextAngle=%f\n", x,y, txt, jTextAlignment,jTextAngle );
   std::string vn(txt);
while (vn.back()==' '||vn.back()=='\n') vn.pop_back();
