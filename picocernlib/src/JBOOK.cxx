...
void hpake_(int* a,float* xe){if (gmapTH1F.find(*a)!=gmapTH1F.end())
	for (int i=0;i<gmapTH1F[*a]->GetNbinsX();i++)
	gmapTH1F[*a]->SetBinError(i+1,xe[i]);
	
	}
void hpak_(int* a,float* xe){if (gmapTH1F.find(*a)!=gmapTH1F.end())
	for (int i=0;i<gmapTH1F[*a]->GetNbinsX();i++)
...
if (gmapTH1F.find(*id)!=gmapTH1F.end())	gmapTH1F[*id]->Write();
}
...
if(in_brackets_sq) vars.back()[1]+=fo.at(i); else
vars.back()[0]+=fo.at(i);
}
