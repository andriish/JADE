...
a small set of basic  sunctions used from CERNLIB 
...
the functionality, see JHIGZ.cxx To embedd the graphics dysplay into a ROOT application,
...
Some used scripts are in the directory scripts2016.
 ...
 The reconstruction software packages were linked to use CERNLIB routines zbug and z  instead of the actually needed rotines for the reconstruction
 from the JADE software. As a result the calibration of part of detector was disabled for the data samles.
 However, this bug had no influence on the results produced previously~\cite{Schieck:2012mp}, as these were using the 
 data reconstructed wuth original software. The bug does not affect Monte Carlo.
...
The setup of MC generators is ommited. The code of the old MC generators is not ported.
